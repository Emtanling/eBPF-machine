...
源码快照化的 Linux/aarch64 运行覆盖具名与随机电路、联合边界、串行复用、机制对照和畸形描述符。作者另行运行的语义审计器重建描述符与电路语义，自行签发的清单检查证据包完整性。案例记录 A；在声明的具体映射服务与无干扰契约下给出**条件性** C 见证；在更多实现前提下支持 P。它没有建立 Linux 报告不可因子分解性 R，也没有建立策略层怪异机器 W；这一边界分离是本文的主要方法论贡献。
...
本文贡献为：（1）带类型的 A/C/P/R/W 论断图谱；（2）包含 E1–E4-D 的有界组合证明义务；（3）一个以 $D_{64,512}$ 为域的 eBPF NAND 解释器校准案例。实证范围仅限一个特权、离线 Linux/aarch64 环境；不是并发部署、无界机器、漏洞、报告不透明性证明或策略级怪异机器。
...
eBPF 案例在源码/对象和串行化前提下支持 P，但没有建立 R 或 W。未来形状定理必须推出接受的**宏闭包**和碰撞的**报告嵌入**；上下文敏感性可破坏任一性质。完备壳理论 [22] 或许能刻画报告细化，但不能生成可达性、重置、路由、同计算链接或策略违反。

## 8. 结论

制品接受与接受后解释使用不同语言。本文区分因果词与输出见证的报告残差，给出唯一单元条件下的因子分解判据，并分解有界组合证明义务。eBPF 案例记录 A，条件性见证 C，并在额外前提下支持固定 64 输入/512 门 NAND 解释器的 P；它既不建立 R，也不建立 W，因此不能据此称为由形状诱导、相对于识别器的怪异机器。

## 伦理与数据可用性

实验在隔离本地 VM 中运行，没有实时钩子或第三方目标，也没有尝试破坏、验证器绕过或提权。

仓库为 <https://github.com/Emtanling/eBPF-machine>；不可变证据位于提交 [`4309069a`](https://github.com/Emtanling/eBPF-machine/tree/4309069a1f94d642d5c1402eb710e089c85059b1) 下的 `results/interpreter/interpreter-final-20260711-02/`。其中包含变体、环境、日志、反汇编、描述符、数据、审计代码与自行签发的完整性清单。其源码是运行当时快照，不是后续最终手稿；清单只防止意外漂移。
