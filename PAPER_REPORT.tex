...
\begin{table*}[t]
\caption{Claim graph and evidence status.}
...
R & \textbf{Report-relative residual:} witnesses at C are jointly covered by an actual computed report cell that fails the behavioral factorization test & auxiliary fixed tuple: established; stock Linux: not established \\
...
The A, C, P, and W status cells in Table~\ref{tab:claim-graph} report the stock eBPF/Linux case. The R cell separates two carriers: $M_{\mathit{linux\_r\_aux\_v1}}$ establishes R for the auxiliary report-producing recognizer and restricted service semantics, while R for the stock Linux verifier remains unestablished.
