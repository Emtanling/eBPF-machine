...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.13\textwidth}p{0.065\textwidth}p{0.12\textwidth}p{0.13\textwidth}p{0.16\textwidth}p{0.065\textwidth}Y@{}}
\toprule
Carrier & A & C & P & R & W & Evidentiary interpretation \\
\midrule
\texttt{wm\_circuit} & recorded & conditional under the declared service contract & supported under source/object, serialization, and frame premises & not established & not established & construction plus audited regression evidence; no computed-cell extractor at the interpreter frontier \\
$M_{\mathit{linux\_r\_aux\_v1}}$ & finite-model acceptance & established within that tuple & not established & established for its custom report & not established & executable finite-model certificate; no refinement or bisimulation claim to stock Linux \\
$M_K$ (\texttt{rac\_single}) & recorded for the frozen tuple & witnessed only in the declared two-history relation & not established & retrospective certificate for an author-declared prune-report & not established & real prune capture plus normalized checks and a manuscript-level wrapper; trace-local, not a Linux functional-report contract \\
\bottomrule
\end{tabularx}
\end{table*}

Each row in Table~\ref{tab:claim-graph} has its own artifact, execution carrier, report interface, and evidence type. The \texttt{wm\_circuit} row carries the paper's A/C/P construction. The auxiliary row supplies a positive R result only for a custom report-producing recognizer. The $M_K$ row supplies a retrospective, trace-local R certificate only for the declared \texttt{rac\_single} operational prune-report tuple. No column may be read by combining cells across rows, and no evaluated row establishes W.
...
\item \textbf{Carrier-separated R evidence.} A fixed auxiliary recognizer/service tuple emits a report and checks a finite report-cell collision with four negative controls. A separate stock-Linux capture records a real exact-level-0 prune and a common suffix with different outcomes. The latter becomes an R instance only after retrospective embedding in the explicitly finite \texttt{rac\_single} carrier and author-declared operational prune-report of Section~5.7. We therefore claim a trace-local operational certificate for that frozen tuple, not a general Linux report result; neither R instance is linked to the interpreter's P evidence.
\end{enumerate}
...
The stock-Linux experiment is author-generated and author-reviewed on one frozen kernel/object tuple; it has not been independently reproduced. The operational report, two-history carrier, observer, and suffix were finalized retrospectively, so the result is vulnerable to selection effects even though the internal checks pass. The bundle binds BTF, configuration, and source-map metadata but does not preserve the target \texttt{verifier.c} function bodies; its restricted shape lemma is not a source-level theorem about general Linux \texttt{states\_equal} behavior. The raw capture-stage analyzer intentionally withholds an R verdict until the bytecode frontier, path correspondence, report contract, and concretization relation are reviewed; the hash-bound integrated checker consumes those later certificates and emits its tuple-specific evidence verdict. The formal R conclusion additionally uses Section~5.7's finite typed wrapper. Treating the raw event, a state hash, or similar verifier-log text alone as R would not satisfy Definition~2.
...
The repository is \url{https://github.com/Emtanling/eBPF-machine}; immutable eBPF-interpreter evidence is at commit \href{https://github.com/Emtanling/eBPF-machine/tree/4309069a1f94d642d5c1402eb710e089c85059b1}{\texttt{4309069a}}, under \path{results/interpreter/interpreter-final-20260711-02/}. The auxiliary report-instance evidence is at commit \href{https://github.com/Emtanling/eBPF-machine/tree/f665b1a2f9a772ee9b2c08a73d116ea283aa5efb}{\texttt{f665b1a}}, under \path{results/linux_r/linux-r-v1/}. The stock-Linux trace evidence is published in the tagged \href{https://github.com/Emtanling/eBPF-machine/tree/V1.0/residuality-auditor/stock-linux-r-proof}{\texttt{V1.0}} snapshot under \path{residuality-auditor/stock-linux-r-proof/}. Its \path{MANIFEST.json}, \path{CHECKSUMS.sha256}, embedded input hashes, raw capture, normalized certificates, proof outputs, and checker sources are co-versioned in that public snapshot. The bundled standard-library verifier emits \path{FROZEN_PROOF_BUNDLE_VERIFIED}. The prepublication archive \path{residuality-auditor-v0.3.0-full.tar.gz} had SHA-256 \path{5fd0a2812c8c8db2fe5508440934817c1cf9293ba0c5df31317e8b38d94a90ec}; V1.0 publishes the frozen proof payload directly rather than duplicating the archive. Any frozen-byte change requires a new version, updated checksums, and re-verification.
