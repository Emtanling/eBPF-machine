...
Nodes C and R must not share one name. We call the accepted-artifact-indexed, same-suffix language $L_{\mathrm{causal}}$. A word enters the report-relative residual language $L_{\mathrm{res}}^R$ only with an actual computed-cell collision under the admissibility conditions below. P and R branch after C. The published weird-machine corpus adds a second separation: programmability and policy effect also branch. We classify an actor-controllable unintended C witness as $\mathsf{WM}_{\mathrm{emergent}}$, refine it independently by P to $\mathsf{WM}_{\mathrm{prog}}$ and by linked W to $\mathsf{WM}_{\mathrm{policy}}$, and reserve the narrower \emph{contract-shape-induced recognizer-relative} class for the linked P/W intersection with R, documented semantics, report conformance, and granularity evidence. This prevents a Turing-completeness demonstration from silently becoming an exploit claim, while retaining weak but policy-relevant machines that do not realize a universal basis.
...
\item \textbf{A cross-family classification lattice.} $\mathsf{WM}_{\mathrm{emergent}}$ is refined independently by programmability and policy effect; the resulting strict lattice explains why page-fault computation, minimalist interrupt machines, exploit-oriented reuse, and report-shape claims occupy different strata. A source-bounded reconstruction covers nine published case families and records missing obligations instead of manufacturing certificates.
...
Language-theoretic security supplies the recognition/interpretation framing \cite{ref1,ref2,ref3,ref4}; object-centric tracing reconstructs state-dependent low-level operation languages \cite{ref10}. Weird-machine work studies unintended computation and exploitability \cite{ref11,ref14}, insecure compilation makes the policy boundary explicit \cite{ref15}, and proof-carrying-code work identifies shadow execution outside a proof model \cite{ref16}. Valid-input computation also appears in well-formed RPM metadata, while vulnerability-flow types derive abstract weird machines \cite{ref27,ref28}. Unlike work that demonstrates rich behavior, our R criterion asks whether a declared computed report actually factors a future-observation quotient at one admitted-artifact frontier.

Published mechanisms span materially different carriers. Code-reuse and data-oriented attacks program existing instructions, signal restoration, non-control data, object dispatch, or exception handling \cite{ref39,ref40,ref41,ref42,ref43}; later work composes individually incompatible language defenses or synthesizes a realizable DOP attack language \cite{ref56,ref57}. DWARF and ELF metadata drive auxiliary interpreters and the runtime loader \cite{ref44,ref45}, while proof-carrying-code work exposes proof-aliased execution outside the proof model \cite{ref16}. The page-fault construction programs IA32 translation and fault machinery without successful instruction dispatch and explicitly presents neither a vulnerability nor an exploit \cite{ref46}. Memory deduplication composed with Rowhammer yields an exploitation machine \cite{ref47}; the FORCEDENTRY technical analysis reconstructs an in-the-wild JBIG2 parser machine with more than 70,000 logical segment commands and a small virtual architecture \cite{ref55}, while the MS11-087 report describes a crafted TrueType font driving the kernel renderer to attacker-chosen memory effects \cite{ref58}. Speculative and microarchitectural work hides or compiles computation into transient or timing state \cite{ref48,ref49,ref50}. Packet-in-packet interpretation crosses protocol layers, interrupt-oriented programming deliberately studies a machine weaker than universal computation, and BGP configurations realize logic circuits under stated assumptions \cite{ref51,ref52,ref53}. These differences motivate the P/W classification lattice in Section~7: computational expressiveness and policy effect are not interchangeable.
...
\begin{table*}[!t]
\caption{Retrospective reconstruction of published weird-machine families; no row is a Definition~2 certificate.}
\label{tab:wm-corpus}
\centering
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\begin{tabularx}{\textwidth}{@{}p{0.13\textwidth}p{0.23\textwidth}p{0.23\textwidth}Y@{}}
\toprule
Family & Boundary artifact / induced engine & Source-reported capability & Retrospective position and missing obligations \\
\midrule
x86 ROP, SROP \cite{ref39,ref40} & stack words or signal frames drive gadgets or signal restoration & general/portable computation; exploit or backdoor paths & candidate $\mathsf{WM}_{\mathrm{prog}}\cap\mathsf{WM}_{\mathrm{policy}}$ in stated exploit contexts; exact C/linkage and R not reconstructed \\
DOP, COOP, CHOP, CLA, DOPPLER \cite{ref41,ref42,ref43,ref56,ref57} & non-control data, objects, exception state, or language transfer drive data flow, dispatch, or unwinding & expressive DOP/COOP; CHOP effects; CLA control hijack; synthesized realizable DOP language & DOP/COOP candidate intersection; CHOP/CLA support policy without new P; DOPPLER strengthens realization; R absent \\
DWARF, ELF, RPM, PCC \cite{ref44,ref45,ref27,ref16} & metadata drives unwind bytecode, RTLD, or package automata; proof abstraction admits unaccounted execution & interpreter/Trojan constructions; proof aliasing or unspecified computation for PCC & DWARF/ELF candidate prog; PCC needs instantiated actor and forbidden effect; R absent \\
IA32 page fault \cite{ref46} & page tables, IDT/TSS, and memory layout drive translation and faults & Turing-complete computation with no successful instruction dispatch; explicitly not vulnerability/exploit & canonical prog candidate without automatic policy status; R absent \\
dedup + Rowhammer; TrueType/JBIG2 \cite{ref47,ref58,ref55} & contents drive dedup/faults, or font/parser commands drive rendering and bitmap logic over kernel/unbounded memory & arbitrary read/write/browser attack; renderer corruption; universal gates and virtual architecture & JBIG2 candidate intersection; dedup/TrueType at least policy candidates without new P; exact linkage and R absent \\
ExSpectre, $\mu$WM, Flexo \cite{ref48,ref49,ref50} & transient or timing state drives hidden gates and compiled circuits & hidden arbitrary computation and high-level compiler with packing demonstration & prog candidate; policy only under separately fixed malware/evasion policy; R absent \\
packet-in-packet \cite{ref51} & inner physical frame in legal outer traffic drives second framing & injection of otherwise prohibited radio frames & policy candidate without P; exact C and R absent \\
interrupt bugdoor \cite{ref52} & interrupt timing/nesting and side effects drive a minimalist machine & platform control from deliberately weak primitives & canonical motivation for $\mathsf{WM}_{\mathrm{policy}}\not\Rightarrow\mathsf{WM}_{\mathrm{prog}}$; R absent \\
BGP circuits \cite{ref53} & configurations and propagation drive gates, clocks, and flip-flops & arbitrary circuits and Turing-equivalent power under assumptions & P-like expressiveness only; no class follows before Unint and, for policy status, W/linkage \\
\bottomrule
\end{tabularx}
\end{table*}
...
\end{aligned}
\end{equation}
enriched with named safety, report-abstraction, and granularity contracts. Here $\mathcal A$, $\mathcal D_A$, and $\mathcal E$ are actor, drive-descriptor, and effect sets. The relation $\mathsf{Induce}$ records which tagged operation words a pair $(a,d)$ can induce through $P_*$; $\mathsf{Drive}\subseteq\mathcal A\times\mathcal D_A\times\mathcal E$ records resulting effects; $\mathsf{Pol}\subseteq\mathcal E$ is the permitted-effect set; and $\mathsf{Int}$ is a named intended-interpretation contract. Intent is not inferred from undocumentedness: documented mechanisms may implement an interpretation outside a stated nominal model. Define $A(M)$ by $P_*\in L_V$;
...
$W(M)$ when $\exists a,d,e.\,(a,d,e)\in\mathsf{Drive}\land e\notin\mathsf{Pol}$.

For classification, let $\mathsf{Ctrl}_C(M)$ require some actor-selected descriptor in $\mathsf{Induce}$ to induce a C witness. Let $\mathsf{Unint}(M)$ mean that this interpretation lies outside $\mathsf{Int}$. Three linkage predicates prohibit combining unrelated co-resident witnesses: $\mathsf{Link}_{CP}$ binds the actor-controlled C witness to the execution family establishing P; $\mathsf{Link}_{CW}$ binds the W effect to the actor/descriptor family inducing C; and $\mathsf{Link}_{PR}$ binds R to the P family. A stronger $\mathsf{Link}_{PW}$ requires the W effect to arise from the same encoded P computation. Define
\begin{equation}
\begin{aligned}
\mathsf{WM}_{\mathrm{emergent}}(M)
&=C(M)\land\mathsf{Ctrl}_C(M)\land\mathsf{Unint}(M),\\
\mathsf{WM}_{\mathrm{prog}}(M)
&=\mathsf{WM}_{\mathrm{emergent}}(M)\land P(M)\\
&\quad{}\land\mathsf{Link}_{CP}(M),\\
\mathsf{WM}_{\mathrm{policy}}(M)
&=\mathsf{WM}_{\mathrm{emergent}}(M)\land W(M)\\
&\quad{}\land\mathsf{Link}_{CW}(M).
\end{aligned}
\end{equation}
...
\medskip
\noindent\textbf{Proposition 4 (strict weird-machine classification lattice).}
The definitions validate
\begin{equation}
\begin{gathered}
...
\emph{Proof.} An actor-driven unintended E1--E4-D NAND interpreter with $\mathsf{Pol}=\mathcal E$ gives $\mathsf{WM}_{\mathrm{prog}}\land\neg\mathsf{WM}_{\mathrm{policy}}$. An actor-driven unintended one-shot state-mediated forbidden effect gives $\mathsf{WM}_{\mathrm{policy}}\land\neg\mathsf{WM}_{\mathrm{prog}}$. Removing W from the latter gives an emergent witness in neither refinement. Assigning exact future-equivalence report cells makes R false without changing C, control, intent, P, or W. The positive implications are definitional. \hfill$\square$
...
\subsection{Retrospective coverage of published weird-machine families}

We used the author-curated Weird Machines HQ index only for discovery \cite{ref54}, then checked original peer-reviewed papers or primary author technical reports in ACM, IEEE, NDSS, USENIX, Project Zero, and the cited conference records. The search was frozen on July~20, 2026. Inclusion required an explicit unexpected-computation construction or a directly argued weird-machine mechanism with an identifiable carrier and drive boundary. We excluded ordinary intended virtual machines, demonstrations whose only fact is Turing completeness, unsupported blog claims, papers using the term only metaphorically, and defenses that introduce no new carrier. Entries sharing a carrier argument were grouped: Framing Signals is under SROP; Phantom Boundaries and 802.3 injection are under packet-in-packet; DOPPLER is under DOP; TrueType is under parser/renderer machines; and proof-carrying code, FORCEDENTRY/JBIG2, and cross-language attacks are explicit below. Intended RarVM or x86-MOV computation, game lists, and accidental-Turing-completeness lists fail the inclusion or Unint gate without an additional boundary argument. This is structured retrospective coverage of named published families, not a claim that every historical exploit has been enumerated.

Table~\ref{tab:wm-corpus} is deliberately evidence-bounded. ``Candidate'' means that the source reports the shown ingredients; it does not mean that we reconstructed Definition~1, E1--E4-D, W, or Definition~2 from source artifacts. None supplies the computed-report cell extractor and unique-fiber factorization evidence required for R here.

The page-fault case is therefore not an exception to the theory: control structures induce computation, placing it on the programmable branch, while the source's no-vulnerability/no-exploit statement withholds the policy branch. Conversely, interrupt and packet-in-packet cases show why requiring P for every policy-relevant weird machine would exclude mechanisms their sources intentionally treat as useful despite limited expressiveness.
...
\bibitem{ref39}
H. Shacham, ``The Geometry of Innocent Flesh on the Bone: Return-into-libc without Function Calls (on the x86),'' in \emph{Proceedings of the 14th ACM Conference on Computer and Communications Security (CCS '07)}, pp. 552--561, 2007, doi: 10.1145/1315245.1315313.

\bibitem{ref40}
E. Bosman and H. Bos, ``Framing Signals---A Return to Portable Shellcode,'' in \emph{2014 IEEE Symposium on Security and Privacy}, pp. 243--258, 2014, doi: 10.1109/SP.2014.23.

\bibitem{ref41}
H. Hu, S. Shinde, S. Adrian, Z. L. Chua, P. Saxena, and Z. Liang, ``Data-Oriented Programming: On the Expressiveness of Non-Control Data Attacks,'' in \emph{2016 IEEE Symposium on Security and Privacy}, pp. 969--986, 2016, doi: 10.1109/SP.2016.62.

\bibitem{ref42}
F. Schuster, T. Tendyck, C. Liebchen, L. Davi, A.-R. Sadeghi, and T. Holz, ``Counterfeit Object-oriented Programming: On the Difficulty of Preventing Code Reuse Attacks in C++ Applications,'' in \emph{2015 IEEE Symposium on Security and Privacy}, pp. 745--762, 2015, doi: 10.1109/SP.2015.51.

\bibitem{ref43}
V. Duta, F. Freyer, F. Pagani, M. Muench, and C. Giuffrida, ``Let Me Unwind That For You: Exceptions to Backward-Edge Protection,'' in \emph{Network and Distributed System Security Symposium (NDSS)}, 2023, doi: 10.14722/ndss.2023.232295.

\bibitem{ref44}
J. Oakley and S. Bratus, ``Exploiting the Hard-Working DWARF: Trojan and Exploit Techniques with No Native Executable Code,'' in \emph{5th USENIX Workshop on Offensive Technologies (WOOT '11)}, 2011. [Online]. Available: \href{https://www.usenix.org/conference/woot11/exploiting-hard-working-dwarf-trojan-and-exploit-techniques-no-native-executable}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref45}
R. Shapiro, S. Bratus, and S. W. Smith, ```Weird Machines' in ELF: A Spotlight on the Underappreciated Metadata,'' in \emph{7th USENIX Workshop on Offensive Technologies (WOOT '13)}, 2013. [Online]. Available: \href{https://www.usenix.org/system/files/conference/woot13/woot13-shapiro.pdf}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref46}
J. Bangert, S. Bratus, R. Shapiro, and S. W. Smith, ``The Page-Fault Weird Machine: Lessons in Instruction-less Computation,'' in \emph{7th USENIX Workshop on Offensive Technologies (WOOT '13)}, 2013. [Online]. Available: \href{https://www.usenix.org/conference/woot13/workshop-program/presentation/bangert}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref47}
E. Bosman, K. Razavi, H. Bos, and C. Giuffrida, ``Dedup Est Machina: Memory Deduplication as an Advanced Exploitation Vector,'' in \emph{2016 IEEE Symposium on Security and Privacy}, pp. 987--1004, 2016, doi: 10.1109/SP.2016.63.

\bibitem{ref48}
J. Wampler, I. Martiny, and E. Wustrow, ``ExSpectre: Hiding Malware in Speculative Execution,'' in \emph{Network and Distributed System Security Symposium (NDSS)}, 2019, doi: 10.14722/ndss.2019.23409.

\bibitem{ref49}
D. Evtyushkin, T. Benjamin, J. Elwell, J. A. Eitel, A. Sapello, and A. Ghosh, ``Computing with Time: Microarchitectural Weird Machines,'' in \emph{Proceedings of the 26th ACM International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS '21)}, pp. 758--772, 2021, doi: 10.1145/3445814.3446729.

\bibitem{ref50}
P.-L. Wang, R. Paccagnella, R. S. Wahby, and F. Brown, ``Bending Microarchitectural Weird Machines towards Practicality,'' in \emph{33rd USENIX Security Symposium}, pp. 1099--1116, 2024. [Online]. Available: \href{https://www.usenix.org/conference/usenixsecurity24/presentation/wang-ping-lun}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref51}
T. Goodspeed, S. Bratus, R. Melgares, R. Shapiro, and R. Speers, ``Packets in Packets: Orson Welles' In-Band Signaling Attacks for Modern Radios,'' in \emph{5th USENIX Workshop on Offensive Technologies (WOOT '11)}, 2011. [Online]. Available: \href{https://www.usenix.org/legacy/event/woot11/tech/final_files/Goodspeed.pdf}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref52}
S. J. Tan, S. Bratus, and T. Goodspeed, ``Interrupt-Oriented Bugdoor Programming: A Minimalist Approach to Bugdooring Embedded Systems Firmware,'' in \emph{Proceedings of the 30th Annual Computer Security Applications Conference (ACSAC '14)}, pp. 116--125, 2014, doi: 10.1145/2664243.2664268.

\bibitem{ref53}
M. Chiesa, L. Cittadini, G. Di Battista, L. Vanbever, and S. Vissicchio, ``Using Routers to Build Logic Circuits: How Powerful Is BGP?,'' in \emph{2013 21st IEEE International Conference on Network Protocols (ICNP)}, pp. 1--10, 2013, doi: 10.1109/ICNP.2013.6733584.

\bibitem{ref54}
Weird Machines HQ, ``What are Weird Machines?,'' curated research index. [Online]. Available: \href{https://weirdmachines.gitlab.io/}{weirdmachines.gitlab.io} (accessed Jul. 20, 2026).

\bibitem{ref55}
I. Beer and S. Gro\ss, ``A Deep Dive into an NSO Zero-Click iMessage Exploit: Remote Code Execution,'' Google Project Zero, Dec. 15, 2021. [Online]. Available: \href{https://projectzero.google/2021/12/a-deep-dive-into-nso-zero-click.html}{Project Zero} (accessed Jul. 20, 2026).

\bibitem{ref56}
S. Mergendahl, N. Burow, and H. Okhravi, ``Cross-Language Attacks,'' in \emph{Network and Distributed System Security Symposium (NDSS)}, 2022, doi: 10.14722/ndss.2022.24078.

\bibitem{ref57}
Y. Ling, G. Rajiv, K. Gopinathan, and I. Sergey, ``Sound and Efficient Generation of Data-Oriented Exploits via Programming Language Synthesis,'' in \emph{34th USENIX Security Symposium (USENIX Security '25)}, pp. 413--429, 2025. [Online]. Available: \href{https://www.usenix.org/conference/usenixsecurity25/presentation/ling}{USENIX} (accessed Jul. 20, 2026).

\bibitem{ref58}
J. Wolf, ``CVE-2011-3402: Windows Kernel TrueType Font Engine Vulnerability (MS11-087),'' CanSecWest presentation, Mar. 8, 2013. [Online]. Available: \href{https://cansecwest.com/slides/2013/Analysis%20of%20a%20Windows%20Kernel%20Vuln.pdf}{CanSecWest} (accessed Jul. 20, 2026).
