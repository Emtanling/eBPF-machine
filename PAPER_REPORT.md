...
For classification, C is the common minimum and P and W form independent refinements. An actor-controllable unintended C witness is an emergent weird machine; adding P gives a programmable weird machine, while adding a linked policy-excluded effect gives a policy-active weird machine. Their intersection, not either branch alone, is required by the narrower report-shape class. A structured retrospective reconstruction of published code-reuse, data-oriented, metadata/proof-model, page-fault, parser, cross-language, cross-layer, interrupt, speculative, microarchitectural, and distributed cases demonstrates why this lattice is necessary. These reconstructions report what the cited sources establish; they are not Definition 2 certificates and do not change the eBPF evidence boundary.
...
Language-theoretic security treats an input boundary as a language-recognition boundary: downstream processing is interpretation, and the input supplies the interpreted program [1]–[4]. Validation and program verification therefore share a recognizer-shaped role. Our question begins after admission. A boundary accepts a code object, metadata record, configuration, proof object, or architectural control structure, and the admitted artifact can induce a language of stateful operations. Which claims are needed before that downstream language can be called residual, programmable, shape-induced, or a weird machine? In this layered view, acceptance, interpretation, and report abstraction answer different questions: membership in $L_V$, behavior in $I$, and grouping by $\mathsf{Report}_V$, respectively. The eBPF instance specializes the general artifact to an accepted program.
...
Nodes C and R must not share one name. We call the accepted-artifact-indexed, same-suffix language $L_{\mathrm{causal}}$. A word enters the report-relative residual language $L_{\mathrm{res}}^R$ only with an actual computed-cell collision under the admissibility conditions below. P and R branch after C. The published weird-machine corpus adds a second separation: programmability and policy effect also branch. We therefore classify an actor-controllable unintended C witness as $\mathsf{WM}_{\mathrm{emergent}}$, refine it independently by P to $\mathsf{WM}_{\mathrm{prog}}$ and by linked W to $\mathsf{WM}_{\mathrm{policy}}$, and reserve the narrower *contract-shape-induced recognizer-relative* class for the linked P/W intersection with R, documented semantics, report conformance, and granularity evidence. This prevents a Turing-completeness demonstration from silently becoming an exploit claim, while retaining weak but policy-relevant machines that do not realize a universal basis.
...
Let $V$ be a recognizer over *boundary artifacts* and $I$ the concrete execution semantics over state carrier $\Sigma_I$ and trace carrier $\mathcal T_I$, including instruction engines, loaders, metadata interpreters, protocol state, architectural mechanisms, stateful services, and the relevant environment. A boundary artifact is any code object, structured data, metadata, configuration, proof object, or control structure whose admission determines subsequent interpretation. We retain $P$ as the artifact metavariable; in the eBPF specialization it is a program. Thus $\mathsf{Tr}_I(P)\subseteq\mathcal T_I$. The accepted artifact language is
...
For $P\in L_V$ and frontier $\ell$, let $\Sigma_{\mathrm{op}}(P)$ be a finite alphabet of boundary-relevant runtime operations, and let $W^\ell_{\mathrm{run}}(P)\subseteq\Sigma_{\mathrm{op}}(P)^*$ be the words that presentation or configuration of $P$ can *induce* from $\ell$ through the declared drive interface. Inducement includes direct execution but does not require it: an ELF relocation table can drive a loader, page tables can drive fault machinery, and a routing configuration can drive distributed transitions. Actor control is not built into C and is checked separately for weird-machine classification. Calling every induced word residual would rename ordinary trace semantics, so node C uses a same-suffix causal test without yet asserting report omission.
...
A defect-induced gap uses behavior that violates the declared recognizer, report, or runtime contract. A documented, safety-preserving witness in $L_{\mathrm{res}}^R$ for a relation the report intends to certify is evidence for a contract-shape gap, not by itself a shape proof: report conformance and granularity must rule out a faulty transformer, join, or extractor. Weirdness is relative to a named intent contract, not synonymous with undocumented behavior or vulnerability: documented mechanisms can compose into an interpretation excluded by the nominal computational model. Section 7 separates the minimum actor-controllable unintended causal machine from its programmable and policy-active refinements. An ordinary intended stateful API can therefore implement a transducer without satisfying any weird-machine classification.
...
Language-theoretic security supplies the recognition/interpretation framing [1]–[4]; object-centric tracing reconstructs state-dependent low-level operation languages [15]. Weird-machine work studies unintended computation and exploitability [16], [17], insecure compilation makes the policy boundary explicit [18], and proof-carrying-code work identifies shadow execution outside a proof model [19]. Valid-input computation also appears in well-formed RPM metadata, while vulnerability-flow types derive abstract weird machines [20], [21]. Unlike work that demonstrates rich behavior, our R criterion asks whether a declared computed report actually factors a future-observation quotient at one admitted-artifact frontier.

Published mechanisms span materially different carriers. Code-reuse and data-oriented attacks program existing instructions, signal restoration, non-control data, object dispatch, or exception handling [35]–[39]; later work composes individually incompatible language defenses or synthesizes a realizable DOP attack language [52], [53]. DWARF and ELF metadata drive auxiliary interpreters and the runtime loader [40], [41], while proof-carrying-code work exposes proof-aliased execution outside the proof model [19]. The page-fault construction programs IA32 translation and fault machinery without successful instruction dispatch and explicitly presents neither a vulnerability nor an exploit [42]. Memory deduplication composed with Rowhammer yields an exploitation machine [43]; the FORCEDENTRY technical analysis reconstructs an in-the-wild JBIG2 parser machine with more than 70,000 logical segment commands and a small virtual architecture [51], while the MS11-087 report describes a crafted TrueType font driving the kernel renderer to attacker-chosen memory effects [54]. Speculative and microarchitectural work hides or compiles computation into transient or timing state [44]–[46]. Packet-in-packet interpretation crosses protocol layers, interrupt-oriented programming deliberately studies a machine weaker than universal computation, and BGP configurations realize logic circuits under stated assumptions [47]–[49]. These differences motivate the P/W classification lattice in Section 7: computational expressiveness and policy effect are not interchangeable.
...
&\mathcal A,\mathcal D_A,\mathcal E,\mathsf{Induce},\mathsf{Drive},\mathsf{Pol},\mathsf{Int}),
\end{aligned}
$$

enriched with named safety, report-abstraction, and granularity contracts. Here $\mathcal A$, $\mathcal D_A$, and $\mathcal E$ are actor, drive-descriptor, and effect sets. The relation $\mathsf{Induce}$ records which tagged operation words a pair $(a,d)\in\mathcal A\times\mathcal D_A$ can induce through the admitted $P_*$; $\mathsf{Drive}\subseteq\mathcal A\times\mathcal D_A\times\mathcal E$ records resulting effects; $\mathsf{Pol}\subseteq\mathcal E$ is the permitted-effect set; and $\mathsf{Int}$ is a named intended-interpretation contract. Intent is therefore not inferred from undocumentedness: documented mechanisms may implement an interpretation outside a stated nominal model.

Define $A(M)$ by $P_*\in L_V$; $C(M)$ by $\exists\ell,\exists w.\,(P_*,\ell,w)\in L_{\mathrm{causal}}(V,I;K_{\mathrm{obs}})$; $P(M)$ when $P_*$ is one fixed accepted bounded interpreter with a same-artifact basis $(P_*,\mathit{reset},G,\mathit{observe},D_G,\mathsf{Adm}_G)$ satisfying E1–E3, $P_*$ discharges E4-D using that basis, and $g$ together with constants $\{0,1\}$ is functionally complete for Boolean circuits; $R(M)$ when $\mathsf{Adm}(P_*,\ell_*,D_R,F_R;K_{\mathrm{obs}})$ holds and some $(P_*,\ell_*,D_R,F_R,a^\#,w)$ is output-witnessed report-relative residual on exactly that tuple; and $W(M)$ when $\exists a,d,e.\,(a,d,e)\in\mathsf{Drive}\land e\notin\mathsf{Pol}$.

For classification, let $\mathsf{Ctrl}_C(M)$ require some actor-selected descriptor in $\mathsf{Induce}$ to induce a C witness. Let $\mathsf{Unint}(M)$ mean that this induced interpretation lies outside $\mathsf{Int}$. Three linkage predicates prohibit combining unrelated co-resident witnesses: $\mathsf{Link}_{CP}$ binds the actor-controlled C witness to the basis/execution family establishing P; $\mathsf{Link}_{CW}$ binds the W effect to the actor/descriptor family inducing C; and $\mathsf{Link}_{PR}$ binds the R collision to the P execution family. A stronger $\mathsf{Link}_{PW}$ requires the W effect to arise from the same encoded P computation. Define the common minimum and its two independent refinements by
...
Let $\mathsf{Doc}(M)$ mean that this linked behavior uses documented semantics and preserves the declared safety contract, $\mathsf{Conf}(M)$ mean that the report implementation conforms to its specified abstraction, and $\mathsf{Gran}(M)$ mean that its R collision is forced by the declared report granularity. Define the narrower report-shape class at the intersection:

$$
\begin{aligned}
\mathsf{WM}_{\mathrm{shape}}(M)={}&\mathsf{WM}_{\mathrm{prog}}(M)
\land\mathsf{WM}_{\mathrm{policy}}(M)\land R(M)\\
...
**Proposition 4 (strict weird-machine classification lattice).** The definitions validate

$$
\mathsf{WM}_{\mathrm{shape}}\Rightarrow
\mathsf{WM}_{\mathrm{prog}}\land\mathsf{WM}_{\mathrm{policy}},
\qquad
\mathsf{WM}_{\mathrm{prog}}\Rightarrow\mathsf{WM}_{\mathrm{emergent}},
\qquad
\mathsf{WM}_{\mathrm{policy}}\Rightarrow\mathsf{WM}_{\mathrm{emergent}}.
$$

Neither refinement implies the other, and none of $\mathsf{WM}_{\mathrm{emergent}}$, $\mathsf{WM}_{\mathrm{prog}}$, or $\mathsf{WM}_{\mathrm{policy}}$ implies R.

*Proof.* An actor-driven unintended E1–E4-D NAND interpreter with $\mathsf{Pol}=\mathcal E$ gives $\mathsf{WM}_{\mathrm{prog}}\land\neg\mathsf{WM}_{\mathrm{policy}}$. An actor-driven unintended one-shot state-mediated forbidden effect gives $\mathsf{WM}_{\mathrm{policy}}\land\neg\mathsf{WM}_{\mathrm{prog}}$. Removing W from the latter gives an emergent witness in neither refinement. Assigning exact future-equivalence report cells to each construction makes R false without changing C, control, intent, P, or W. The positive implications are definitional. ∎

The lattice is the smallest extension needed by the corpus below: it preserves C as a machine-level minimum, does not demand Turing completeness from every exploit mechanism, and does not turn expressiveness alone into a policy claim.

### 7.2 Retrospective coverage of published weird-machine families

We used the author-curated Weird Machines HQ index only for discovery [50], then checked original peer-reviewed papers or primary author technical reports in ACM, IEEE, NDSS, USENIX, Project Zero, and the cited conference records. The search was frozen on July 20, 2026. Inclusion required an explicit unexpected-computation construction or a directly argued weird-machine mechanism with an identifiable carrier and drive boundary. We excluded ordinary intended virtual machines, demonstrations whose only fact is Turing completeness, unsupported blog claims, papers using “weird machine” only metaphorically, and defenses that introduce no new carrier. Index entries were grouped when their carrier argument was the same: Framing Signals is under SROP; Phantom Boundaries and 802.3 injection are under packet-in-packet; DOPPLER is under DOP; TrueType is under parser/renderer machines; and proof-carrying code, FORCEDENTRY/JBIG2, and cross-language attacks are explicit below. Intended RarVM or x86-MOV computation, game lists, and accidental-Turing-completeness lists fail the inclusion or Unint gate without an additional boundary argument. This is structured retrospective coverage of named published families, not a claim that every historical exploit has been enumerated.

The table is deliberately evidence-bounded. “Candidate” means that the cited source reports the ingredients shown; it does **not** mean that we reconstructed Definition 1, E1–E4-D, W, or Definition 2 from source artifacts. In particular, none of these papers supplies the computed-report cell extractor and unique-fiber factorization evidence required for R here.

| Published family | Boundary artifact and induced engine | Source-reported capability | Retrospective lattice position; unresolved obligations |
|---|---|---|---|
| x86 ROP and SROP [35], [36] | corrupted stack words or signal frames drive existing gadgets or kernel signal restoration | general/portable computation and concrete exploit or backdoor paths | candidate $\mathsf{WM}_{\mathrm{prog}}\cap\mathsf{WM}_{\mathrm{policy}}$ in the stated exploit contexts; exact C/linkage and R are not reconstructed |
| DOP, COOP, CHOP, CLA, and DOPPLER [37]–[39], [52], [53] | non-control data, counterfeit objects, exception state, or cross-language transfer drives data flow, dispatch, or unwinding | expressive DOP/COOP; CHOP effects; CLA control hijack across individually blocking boundaries; synthesized realizable DOP language | DOP/COOP are candidate intersections; CHOP/CLA support policy without a new P proof; DOPPLER strengthens the family-level realization claim; R absent |
| DWARF, ELF, RPM, and PCC [40], [41], [20], [19] | metadata drives unwind bytecode, RTLD, or package-manager automata; proof abstraction admits unaccounted execution | Turing-complete or bounded interpreters and Trojan/exploit contexts; proof-aliasing/unspecified computation for PCC | DWARF/ELF are candidate prog; PCC is only emergent/policy after actor and forbidden effect are instantiated; R absent |
| IA32 page-fault machine [42] | page tables, IDT/TSS state, and fault-triggering memory layout drive translation, page faults, and double faults | Turing-complete computation with no successful CPU-instruction dispatch; explicitly neither vulnerability nor exploit | canonical $\mathsf{WM}_{\mathrm{prog}}$ candidate without an automatic $\mathsf{WM}_{\mathrm{policy}}$ claim; R absent |
| deduplication plus Rowhammer; TrueType/JBIG2 [43], [54], [51] | chosen contents drive deduplication/physical faults, or font/parser commands drive rendering and bitmap logic over kernel/unbounded memory | cross-layer arbitrary read/write/browser attack; renderer corruption; universal gates, virtual architecture, and an in-the-wild sandbox exploit | JBIG2 is a candidate intersection; dedup/TrueType are at least policy candidates without new P proofs; exact C/linkage and R are not reconstructed |
| ExSpectre, $\mu$WM, and Flexo [44]–[46] | speculative paths or timing/microarchitectural state drive hidden gates and compiled circuits | hidden arbitrary computation, circuit construction, and a high-level compiler with malware-packing demonstrations | candidate $\mathsf{WM}_{\mathrm{prog}}$; policy-active in a separately fixed malware/evasion policy; R absent |
| packet-in-packet interpretation [47] | an inner physical-layer frame embedded in legal outer traffic drives a second framing interpretation | cross-layer injection of otherwise prohibited radio frames | candidate $\mathsf{WM}_{\mathrm{policy}}$ without a P requirement; exact C and R absent |
| interrupt-oriented bugdoor [48] | interrupt timing/nesting and firmware side effects drive a minimalist embedded machine | platform control from deliberately weak primitives; the paper contrasts utility with maximal expressiveness | canonical motivation for $\mathsf{WM}_{\mathrm{policy}}\not\Rightarrow\mathsf{WM}_{\mathrm{prog}}$; R absent |
| BGP logic circuits [49] | router configurations and message propagation drive gates, clocks, and flip-flops | arbitrary circuits and Turing-equivalent power under stated assumptions | P-like expressiveness alone: no weird-machine class follows until $\mathsf{Unint}$ and, for policy status, W/linkage are established |

This reconstruction answers the page-fault boundary directly. It is not an exception to the theory: it occupies the programmable branch because control structures induce the computation, while the source's no-vulnerability/no-exploit statement withholds the policy branch. Conversely, interrupt and packet-in-packet cases explain why requiring P for every policy-relevant weird machine would exclude mechanisms their source literature intentionally treats as useful despite limited expressiveness.
...
### 7.4 eBPF weird-machine status and a future shape theorem

The `wm_circuit` carrier supports P under its source/object and serialization premises but establishes neither $\mathsf{Ctrl}_C/\mathsf{Unint}$ nor W or R, so P alone does not classify it. The frozen V1 $M_{\mathrm{Linux}}$ record has a separate $M_K^{\mathrm{legacy}}=M_{\mathrm{adapter}}$ finite-model factorization result, but the current assessment of the V1 exact query is `UNKNOWN`, not R. The V2 $M_{\mathrm{Linux}}^{\mathrm{V2}}$ record establishes proof-bound `NONFACTORING` only for `EXACT_STOCK_R_V2_QUERY`; its report carrier, object, and witness are still not the interpreter carrier. The auxiliary $M_{\mathit{linux\_r\_aux\_v1}}$ establishes R only for its fixed custom report and restricted service semantics. Because the auxiliary and V2 R carriers differ from the interpreter's P carrier, $\mathsf{Link}_{PR}$ forbids combining them. No evaluated eBPF model therefore establishes $\mathsf{WM}_{\mathrm{emergent}}$, $\mathsf{WM}_{\mathrm{prog}}$, $\mathsf{WM}_{\mathrm{policy}}$, or $\mathsf{WM}_{\mathrm{shape}}$. A shape theorem must derive **macro closure** of acceptance, actor/linkage obligations, and **report embedding** of the collision on one carrier; context sensitivity can defeat any of them. Complete-shell theory [22] may characterize report refinement, but cannot create reachability, reset, routing, linkage, or policy violation.

---

## 8. Conclusion

At a layered LangSec boundary, artifact recognition, induced interpretation, report factorization, bounded programmability, and policy effect are distinct: $R\Rightarrow C\Rightarrow A$ and $P\Rightarrow C\Rightarrow A$, while Proposition 3 gives finite countermodels for the other listed implications. Proposition 4 places actor-controllable unintended C at the base of a strict weird-machine lattice, with independent programmable and policy-active refinements. The retrospective corpus explains x86 reuse, metadata and proof-model gaps, page-fault computation, parser and cross-language machines, minimalist exploit machines, cross-layer interpretation, microarchitectural machines, and BGP without equating Turing completeness, exploitation, or report non-factorization.

The `wm_circuit` carrier records A, conditionally witnesses C, and supports P for a fixed 64-input/512-gate NAND interpreter, but establishes neither R nor W or the intent/control predicates needed for classification. The auxiliary tuple independently establishes R for its custom report. Stock-Linux V1 supplies a real exact-level-0 prune event and differing common-suffix samples, but without a must-outcome proof its evidence-bounded operational-prune query remains `UNKNOWN`. Stock-R V2 supplies a proof and history-case binding for a separate controlled witness and establishes `NONFACTORING` only for `EXACT_STOCK_R_V2_QUERY`; the generic checker preserves that exact boundary and rejects the tested claim lifts. CRL transports that exact source certificate to two separate generated targets, but only as exact `AT(target)` certificates, not as a family theorem. Neither the auxiliary, V2, nor CRL carrier supplies P or W or links to the interpreter, so the eBPF evidence establishes no weird-machine class, verifier unsoundness, vulnerability, or universal necessity theorem.
...
[35] H. Shacham, “The Geometry of Innocent Flesh on the Bone: Return-into-libc without Function Calls (on the x86),” in *Proceedings of the 14th ACM Conference on Computer and Communications Security (CCS ’07)*, pp. 552–561, 2007, doi: 10.1145/1315245.1315313.

[36] E. Bosman and H. Bos, “Framing Signals—A Return to Portable Shellcode,” in *2014 IEEE Symposium on Security and Privacy*, pp. 243–258, 2014, doi: 10.1109/SP.2014.23.

[37] H. Hu, S. Shinde, S. Adrian, Z. L. Chua, P. Saxena, and Z. Liang, “Data-Oriented Programming: On the Expressiveness of Non-Control Data Attacks,” in *2016 IEEE Symposium on Security and Privacy*, pp. 969–986, 2016, doi: 10.1109/SP.2016.62.

[38] F. Schuster, T. Tendyck, C. Liebchen, L. Davi, A.-R. Sadeghi, and T. Holz, “Counterfeit Object-oriented Programming: On the Difficulty of Preventing Code Reuse Attacks in C++ Applications,” in *2015 IEEE Symposium on Security and Privacy*, pp. 745–762, 2015, doi: 10.1109/SP.2015.51.

[39] V. Duta, F. Freyer, F. Pagani, M. Muench, and C. Giuffrida, “Let Me Unwind That For You: Exceptions to Backward-Edge Protection,” in *Network and Distributed System Security Symposium (NDSS)*, 2023, doi: 10.14722/ndss.2023.232295.

[40] J. Oakley and S. Bratus, “Exploiting the Hard-Working DWARF: Trojan and Exploit Techniques with No Native Executable Code,” in *5th USENIX Workshop on Offensive Technologies (WOOT ’11)*, 2011. [Online]. Available: https://www.usenix.org/conference/woot11/exploiting-hard-working-dwarf-trojan-and-exploit-techniques-no-native-executable (accessed Jul. 20, 2026).

[41] R. Shapiro, S. Bratus, and S. W. Smith, “‘Weird Machines’ in ELF: A Spotlight on the Underappreciated Metadata,” in *7th USENIX Workshop on Offensive Technologies (WOOT ’13)*, 2013. [Online]. Available: https://www.usenix.org/system/files/conference/woot13/woot13-shapiro.pdf (accessed Jul. 20, 2026).

[42] J. Bangert, S. Bratus, R. Shapiro, and S. W. Smith, “The Page-Fault Weird Machine: Lessons in Instruction-less Computation,” in *7th USENIX Workshop on Offensive Technologies (WOOT ’13)*, 2013. [Online]. Available: https://www.usenix.org/conference/woot13/workshop-program/presentation/bangert (accessed Jul. 20, 2026).

[43] E. Bosman, K. Razavi, H. Bos, and C. Giuffrida, “Dedup Est Machina: Memory Deduplication as an Advanced Exploitation Vector,” in *2016 IEEE Symposium on Security and Privacy*, pp. 987–1004, 2016, doi: 10.1109/SP.2016.63.

[44] J. Wampler, I. Martiny, and E. Wustrow, “ExSpectre: Hiding Malware in Speculative Execution,” in *Network and Distributed System Security Symposium (NDSS)*, 2019, doi: 10.14722/ndss.2019.23409.

[45] D. Evtyushkin, T. Benjamin, J. Elwell, J. A. Eitel, A. Sapello, and A. Ghosh, “Computing with Time: Microarchitectural Weird Machines,” in *Proceedings of the 26th ACM International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS ’21)*, pp. 758–772, 2021, doi: 10.1145/3445814.3446729.

[46] P.-L. Wang, R. Paccagnella, R. S. Wahby, and F. Brown, “Bending Microarchitectural Weird Machines towards Practicality,” in *33rd USENIX Security Symposium*, pp. 1099–1116, 2024. [Online]. Available: https://www.usenix.org/conference/usenixsecurity24/presentation/wang-ping-lun (accessed Jul. 20, 2026).

[47] T. Goodspeed, S. Bratus, R. Melgares, R. Shapiro, and R. Speers, “Packets in Packets: Orson Welles’ In-Band Signaling Attacks for Modern Radios,” in *5th USENIX Workshop on Offensive Technologies (WOOT ’11)*, 2011. [Online]. Available: https://www.usenix.org/legacy/event/woot11/tech/final_files/Goodspeed.pdf (accessed Jul. 20, 2026).

[48] S. J. Tan, S. Bratus, and T. Goodspeed, “Interrupt-Oriented Bugdoor Programming: A Minimalist Approach to Bugdooring Embedded Systems Firmware,” in *Proceedings of the 30th Annual Computer Security Applications Conference (ACSAC ’14)*, pp. 116–125, 2014, doi: 10.1145/2664243.2664268.

[49] M. Chiesa, L. Cittadini, G. Di Battista, L. Vanbever, and S. Vissicchio, “Using Routers to Build Logic Circuits: How Powerful Is BGP?,” in *2013 21st IEEE International Conference on Network Protocols (ICNP)*, pp. 1–10, 2013, doi: 10.1109/ICNP.2013.6733584.

[50] Weird Machines HQ, “What are Weird Machines?,” curated research index. [Online]. Available: https://weirdmachines.gitlab.io/ (accessed Jul. 20, 2026).

[51] I. Beer and S. Groß, “A Deep Dive into an NSO Zero-Click iMessage Exploit: Remote Code Execution,” Google Project Zero, Dec. 15, 2021. [Online]. Available: https://projectzero.google/2021/12/a-deep-dive-into-nso-zero-click.html (accessed Jul. 20, 2026).

[52] S. Mergendahl, N. Burow, and H. Okhravi, “Cross-Language Attacks,” in *Network and Distributed System Security Symposium (NDSS)*, 2022, doi: 10.14722/ndss.2022.24078.

[53] Y. Ling, G. Rajiv, K. Gopinathan, and I. Sergey, “Sound and Efficient Generation of Data-Oriented Exploits via Programming Language Synthesis,” in *34th USENIX Security Symposium (USENIX Security ’25)*, pp. 413–429, 2025. [Online]. Available: https://www.usenix.org/conference/usenixsecurity25/presentation/ling (accessed Jul. 20, 2026).

[54] J. Wolf, “CVE-2011-3402: Windows Kernel TrueType Font Engine Vulnerability (MS11-087),” CanSecWest presentation, Mar. 8, 2013. [Online]. Available: https://cansecwest.com/slides/2013/Analysis%20of%20a%20Windows%20Kernel%20Vuln.pdf (accessed Jul. 20, 2026).
