...
A source-snapshotted Linux/aarch64 run covers named and random circuits, joint bounds, serial reuse, mechanism controls, and malformed descriptors. A separate author-run semantic auditor reconstructs descriptors and circuit semantics, and a self-issued manifest checks bundle integrity. This interpreter carrier records A, gives a conditional C witness under the declared concrete-service/no-interference contract, and supports P under additional implementation premises; it establishes neither R nor W. A fixed auxiliary executable instance establishes $R(M_{\mathit{linux\_r\_aux\_v1}})$ for its own computed report. Separately, a local stock-Linux/aarch64 experiment on a different accepted XDP object captures a successful exact-level-0 `states_equal` check followed by an `is_state_visited` prune, binds the two recorded prefixes to one retained-state representative, and records different observations under one common suffix. Embedded in a retrospectively declared, hash-bound two-history carrier with an author-declared operational prune-report, those records satisfy Definition 2 for that tuple. We describe this result as a trace-local operational-prune-relative certificate, not as evidence that Linux specifies a complete functional report or as a theorem over general Linux executions. The carriers are not combined: neither R result is linked to the interpreter's P evidence, and no result establishes W or a weird machine.
...
| Carrier | A | C | P | R | W | Evidentiary interpretation |
|---|---|---|---|---|---|---|
| `wm_circuit` interpreter | recorded | conditional under the declared service/no-interference contract | supported under source/object, serialization, and frame premises | not established | not established | construction plus audited regression evidence; no computed-cell extractor at the interpreter frontier |
| $M_{\mathit{linux\_r\_aux\_v1}}$ | established within the finite custom recognizer | established within that tuple | not established | established for its computed custom report | not established | executable finite-model certificate; no refinement or bisimulation claim to stock Linux |
| $M_K$ (`rac_single`) | recorded for the frozen object/kernel tuple | witnessed only in the declared two-history relation | not established | retrospectively certified for the author-declared operational prune-report | not established | real prune capture plus normalized, hash-bound checks and a manuscript-level one-step wrapper; trace-local and not a Linux functional-report contract |

Each row has its own artifact, execution carrier, report interface, and evidence type. The `wm_circuit` row carries the paper's A/C/P construction. The auxiliary row supplies a positive R result only for a custom report-producing recognizer. The $M_K$ row supplies a retrospective, trace-local R certificate only for the declared `rac_single` operational prune-report tuple. No column may be read by combining cells across rows, and no evaluated row establishes W.
...
4. **Carrier-separated R evidence.** A fixed auxiliary recognizer/service tuple emits a report and checks a finite report-cell collision with four negative controls. A separate stock-Linux capture records a real exact-level-0 prune and a common suffix with different outcomes. The latter becomes an R instance only after retrospective embedding in the explicitly finite `rac_single` carrier and author-declared operational prune-report of Section 5.7. We therefore claim a trace-local operational certificate for that frozen tuple, not a general Linux report result; neither R instance is linked to the interpreter's P evidence.

The empirical scope is one privileged, offline Linux/aarch64 kernel build, two claim-bearing accepted eBPF artifacts, three additional accepted interpreter-control variants, and one accepted auxiliary custom-recognizer artifact. The result is not a general Linux report-opacity theorem, concurrent deployment result, artifact-parametric compiler, unbounded machine, vulnerability, or policy-level weird machine.
...
Instantiating Definition 2 requires an extractor for actual computed cells, a declared concretization, a unique-cell partition on the chosen fiber (or a separately defined alternative report map), and the remaining admissibility premises. The retained `wm_circuit` Linux log does not provide these objects, so that interpreter carrier gives only a conditional $L_{\mathrm{causal}}$ witness under its declared service contract and does not establish $L_{\mathrm{res}}^R$. Section 5.7 instantiates those objects for a different, finite `rac_single` carrier.
...
On the archived bundle, a separately implemented, author-run checker reconstructs reachability, unique-cell coverage, the quotient, and the factorization verdict without importing the model implementation, and emits the established auxiliary-R verdict. Separately, one regression test builds the same deterministic model bundle twice and byte-compares its five formal JSON files; this is a determinism test, not experimental evidence. The certificate is executable finite-model evidence, not a machine-checked proof. The retained kernel calibration has four oracle rows for the two assignments $(0,1)$ and $(1,1)$; it calibrates those two restricted service outcomes only. It proves no refinement or bisimulation between $I_{\mathit{hash}}$ and Linux and extracts no stock-verifier cell. The auxiliary certificate therefore remains independent of the stock-Linux certificate below. Its bundle is `results/linux_r/linux-r-v1/`.

### 5.7 Post-hoc stock-Linux trace certificate

This subsection analyzes a completed local stock-Linux experiment as a retrospective trace certification. The kernel capture is primary evidence that one prune event and two runtime outcomes occurred on the frozen tuple. The integrated and frozen-bundle checkers validate the declared evidence relationships offline; their regression suites are not substitutes for the capture, and neither layer converts the author-declared operational prune-report into a documented Linux functional-report contract.

To apply Definition 2 without reusing the auxiliary semantics, we fix the hash-bound, evidence-restricted carrier
...
Under the jointly defined word $a_B$, the recorded **program-level success bits** are 1 from $\sigma_0$ and 0 from $\sigma_1$; they are not raw `bpf_map_update_elem` returns (0 on success and negative on failure). The unequal outputs imply $p_0\not\sim_{D_K}p_1$ directly from the definition of $\sim_{D_K}$, so $\beta_{D_K}(\sigma_0)=Q_0\ne Q_1=\beta_{D_K}(\sigma_1)$ while the operational report map assigns both concrete states to the same cell. Acceptance and identity, reachability and path, direct membership, session completeness, unique-cell, quotient, factorization, and hash gates all pass. Together with the explicitly typed one-step construction above, these facts establish $\mathsf{Adm}(P_R,\ell_K,D_K,F_K;K^K_{\mathrm{obs}})$ and satisfy Definition 2. Therefore

$$
R(M_K)
$$

holds for the declared finite frozen carrier. We report this as a **retrospective, trace-local, operational-prune-relative certificate**. The integrated checker emits `STOCK_LINUX_R_ESTABLISHED_FOR_FROZEN_TUPLE`, but that identifier denotes passage of the hash-bound gates encoded for this tuple; it is not a claim that Linux exposes or violates a documented complete functional report. The typing of $I_K,D_K,K^K_{\mathrm{obs}}$ and the soundness argument above are the manuscript-level formal wrapper for the one-step records, not a mechanically checked theorem over the full Linux execution space. The bundled adapter presents each prefix case as self-looping when recomputing its stable quotient. Because the first $a_B$ output already differs, its refinement splits the states at depth one; replacing those self-loops by the terminal transitions above preserves that split and the independently extracted operational report map. We do not identify the two quotient models. The required $\beta$-inequality and output-witness clause are proved by $a_B$; no longer-word result is imported.

The certificate is carrier-, tuple-, observer-, suffix-, and report-relative. Because those choices were finalized after the distinguishing capture was available, it supports an existence statement for the declared retrospective tuple but not an independently precommitted or population-level Linux claim. It does not generalize $I_K$ to uncaptured inputs, continuations, or reachable states; generalize to other kernels, configurations, BTFs, compiler outputs, or eBPF objects; characterize general `states_equal` semantics; prove verifier unsoundness, a vulnerability, P, W, or a weird machine; or show that map occupancy belongs to a Linux-specified complete functional report. The concretization argument is restricted to constructed witnesses compatible with the captured State V2 records and locally future-observable fields. Because $P_R\ne P_U$, the certificate cannot be combined with the interpreter's P certificate to satisfy $\mathsf{Link}$. The frozen evidence is under `residuality-auditor/stock-linux-r-proof/`.
...
The stock-Linux experiment is author-generated and author-reviewed on one frozen kernel/object tuple; it has not been independently reproduced. The operational report, two-history carrier, observer, and suffix were finalized retrospectively, so the result is vulnerable to selection effects even though the internal checks pass. The bundle binds BTF, configuration, and source-map metadata but does not preserve the target `verifier.c` function bodies; its restricted shape lemma is not a source-level theorem about general Linux `states_equal` behavior. The raw capture-stage analyzer intentionally withholds an R verdict until the bytecode frontier, path correspondence, report contract, and concretization relation are reviewed; the hash-bound integrated checker consumes those later certificates and emits its tuple-specific evidence verdict. The formal R conclusion additionally uses Section 5.7's finite typed wrapper. Treating the raw event, a state hash, or similar verifier-log text alone as R would not satisfy Definition 2.
