...
`results/interpreter/interpreter-v1-20260710-04/` passed its independent semantic audit and
self-issued SHA-256 integrity check. It records 38,533 JSONL rows: 26,488 per-gate traces,
12,037 successful run records, and eight malformed-core controls. A fixed eBPF artifact `P_U`
...
suite covers named-circuit truth tables, a fixed-seed random corpus, a 512-gate chain, a joint
64-input/512-gate/578-wire boundary, a zero-gate boundary, serial alternating-invocation
regression, and mechanism controls. The
...
5. **A precision taxonomy.** We separate verifier-unresolved observations, relational
   ...
   Linux artifact provides paired-run and local-log evidence but does not instantiate the first
   without a `Report_log`-to-cell interpretation and its coverage/transfer-soundness proof; the
   second is conditional; the third is claimed only when a concrete strict-inequality witness is
   shown.
...
> core descriptor `d` and its mandatory, range-checked but host-only output list `pi_out`. The output
> list is not part of `d` and is not consumed by `P_U`. Let `Conf_U` be the set of map configurations over
...
> The host may apply `pi_out` only to a successful `WireObs(mu)`.
>
> A fixed artifact `P_U` discharges **E4-D** for `D_{M,N}` when:
>
> 1. `P_U in L_V` independently of the host configuration;
> 2. for every `d in D_{M,N}` and `x in {0,1}^{m(d)}`, under the gate environment required by
>    Definition 7, invocation from `Enc_U(d,x)` first establishes the base frame
>    `WIRES[0]=0`, `WIRES[1]=1`, and `WIRES[2+j]=x_j` for every `0<=j<m(d)` before callback
>    position `0`; it preserves those primary-input cells through that entry, schedules and
>    successfully completes exactly the callback positions `i=0,...,n(d)-1` in order and without
>    per-gate intervention by an external oracle, then reports
>    `PhysRun_U(Enc_U(d,x))=(OK,n(d),mu)` for some physical state `mu`. This loop-control and
>    base-frame clause does not itself assert `WireObs(mu)=Eval(d,x)`;
...
>    `VM_CONTROL`, `VM_TRACE`, and the gate map. It excludes every nonparticipating `P_U`
>    invocation and every nonparticipating host reader or writer throughout setup, invocation, and
>    readback, so only the serializing client can observe the encoded descriptor, inputs, gate
>    state, trace, or result for the whole transaction;
> 4. on a valid configuration, each successful iteration `i` checks canonical form, reads exactly
>    its two designated source wires, invokes the embedded uniform residual gate, writes its
>    resulting `g` bit only to `2+m+i` and declared audit cells, and preserves earlier wire cells;
>    the gate-totality/environment premise determining success is supplied by Definition 7;
...
update, trace update, gate-map update, or observation by another `P_U` invocation or any
nonparticipating host reader/writer; the compiler barrier in the implementation is not a
concurrency lock. ∎
...
*Proof.* By induction on `t`. At `t=0`, E4-D clause 2 establishes exactly the constant/input
prefix used to initialize `Eval(d,x)`. For the induction step, the descriptor bound makes both
sources of gate `t` earlier cells; Lemma 2 and the gate correctness of Definition 7 supply `g` at
the canonical destination, while the frame clause preserves the induction hypothesis. ∎
...
*Proof.* Clause 2 of E4-D gives exactly `n(d)` successful ordered iterations and terminal `OK`
control. Lemma 2 identifies each local update with the Definition 7 gate, and Lemma 3 at
`t=n(d)` gives `WireObs(mu)=Eval(d,x)`; Definition 1 supplies the conditional safety conclusion.
∎
...
> **Definition 9 (persistent-alternative certificate).** Fix `d in D_{M,N}` and an admissible
> host-side projection `pi_out`. Let `Pi_{U,d,pi_out}` be the **open input-parametric interface**
> whose invocation on any `x in {0,1}^{m(d)}` installs `Enc_U(d,x)`, invokes `P_U` under the E4-D
> transaction discipline, and applies `pi_out` only to the successful wire observation. Thus `d`
> and `pi_out` are frozen but `x` ranges over the interface domain; `Pi_{U,d,pi_out}` is not one
> execution from a frozen `Enc_U(d,x)`. Under the premises of Theorem 1, this interface is total
> and deterministic with function `f_{d,pi_out}(x)=pi_out(Eval(d,x))`.
>
> Alongside the interface extractor `Extract_Q` of Definition 6, fix a local extractor
> `Extract_Q^loc` over the same complete report and vocabulary for `Pi_{U,d,pi_out}`. It produces
> formulas for each designated gate frontier and exact formulas for explicit copy/fan-out
> operations in their typed variable contexts. After consistent variable renaming and compatible
> natural joins, let `U_C` be the complete typed context of primary inputs `x`, internal wires `z`,
> and final output `y`, and let `K_C in Form_Q(U_C)` be their conjunction over those variables. Let
> `Loc_C=exists z.K_C in Form_Q(U_io)`. Let
> `Γ_Q(Pi_{U,d,pi_out}) subseteq Form_Q(U_io)` be the extracted whole-interface theory. A
> *persistent-alternative certificate* for a selected nonconstant `f_{d,pi_out}` consists of:
>
> 1. a pair `(x,y')` with `y'!=f_{d,pi_out}(x)` satisfying `Loc_C`; and
> 2. a checked dominance obligation `Loc_C models_Q Γ_Q(Pi_{U,d,pi_out})`, meaning every pair
>    admitted by the composed local summaries is also admitted by the whole-interface report theory.
>
...
> **Theorem 2 (conditional Q-certificate opacity).** Under Theorem 1 for fixed `d` and an
> admissible fixed `pi_out`, let `f_{d,pi_out}` be nonconstant, let its graph be Q-expressible,
> and let `Extract_Q` be sound for `Pi_{U,d,pi_out}`. If a valid persistent-alternative certificate
> exists for `Pi_{U,d,pi_out}`, then `Pi_{U,d,pi_out}` is Q-certificate-opaque for
> `f_{d,pi_out}`.

*Proof.* The persistent pair `(x,y')` is a model of `Loc_C`. Dominance makes it a model of the
whole-interface theory `Γ_Q(Pi_{U,d,pi_out})`. Because `y'!=f_{d,pi_out}(x)`, it is not a model
of the target graph.
...
| `Env` | dedicated preallocated non-LRU map; whole-transaction mutual exclusion across every shared map and every nonparticipating host reader or writer; fixed flags/program type; and the kernel/architecture captured in the interpreter run's `environment.txt` |
...
|---|---|---|
| recognized boundary | in-kernel verifier; concrete eBPF/JIT plus map/helper runtime; exact environment | `interpreter-v1-20260710-04` verifier log and environment record |
...
| E4-D fixed interpreter | one fixed `wm_circuit` reads normalized core-gate, input, and control maps; it validates canonical SSA and dispatches every gate through `G0`; host code separately parses WMC1 and projects selected output wires | source snapshot and `interpreter-v1-20260710-04` object/audit |
...
| bounded circuit configuration domain | the host WMC1 encoder plus independent oracle drive named circuits, a fixed-seed random corpus, zero-gate, deep 512-gate, and joint 64-input/512-gate/578-wire descriptors | regenerated source/descriptors/corpus and interpreter JSONL audit |
...
is parsed outside the kernel; its mandatory host-only output list is a projection and is never read by
...
`interpreter-v1-20260710-04` rebuilt the fixed interpreter, regenerated WMC1 descriptors from
symbolic circuit descriptions, and passed both `interpreter audit: ok` and `interpreter
provenance: ok`. It contains 38,533 JSONL rows: 26,488 per-gate records, 12,037 successful
...
23,776 traces; the deep boundary contributes two 512-gate runs and 1,024 traces; the joint ABI
boundary contributes two 64-input/512-gate runs, exercises wire 577, and contributes 1,024
traces; the zero-gate descriptor contributes one run; the serial alternation contributes exactly
10,000 runs; and each of the three controls contributes 39 runs and 166 traces.

The audit independently recompiles every named source JSON and both boundary generators
byte-for-byte, decodes every WMC1 descriptor, recomputes complete wire vectors and projected
outputs, regenerates the fixed-seed corpus byte-for-byte, checks all boundary-gate traces and all
eight negative cases, and requires each JSONL file's runtime BPF program tag to match the tag
captured from its preserved variant object. The suite covers normal,
...
nonparticipating host reader/writer, key reuse, reset failure, map-type changes, and version-dependent helper semantics.
...
execution evidence is recorded in `interpreter-v1-20260710-04`. It does not establish artifact-
...
persistent-alternative certificate that the artifact does not supply. Whole-transaction mutual exclusion across the
entire shared map set is essential: another invocation or any nonparticipating host reader/writer is outside the theorem
and experiments. The audited `interpreter-v1-20260710-04` result is finite regression evidence,
not a universal enumeration. Named-circuit tests, bounded random corpora, deep and joint-boundary cases are not an
