# Research Report: After Acceptance: A Claim Graph for Residual Machines, Calibrated on eBPF
...
We then state proof obligations for a uniformly controlled, observable, resettable gate and one fixed accepted interpreter that consumes an independently bounded circuit description. Prefix induction yields bounded composition when exact scheduling and frame conditions hold. An eBPF calibration case establishes nodes A, C, and P. A dedicated, preallocated, non-LRU two-entry hash map implements NAND: after reset to one live sentinel, bits select existing- or fresh-key updates, and the second update’s success predicate is the output. One fixed accepted program consumes canonical NAND DAGs with at most 64 inputs, 512 gates, and 578 live wires.
...
Let $V$ be a recognizer over program artifacts and $I$ the concrete execution semantics, including the instruction engine, helpers, stateful services, and relevant environment. The accepted artifact language is
...
The projection $\rho_{\mathrm{obs}}$ selects the state candidate; $\mathsf{Obs}$ fixes the complete visible observation trace; $\mathsf{Slice}$ conservatively includes every non-selected component read by the suffix or observer; and $\mathsf{Env}$ fixes the same relevant resource configuration, schedule, nondeterminism, and external-interference choice for both executions. Write $\mathsf{ctx}_w(\sigma)$ for the suffix-read components other than $\rho_{\mathrm{obs}}(\sigma)$.

When defined, write $\llbracket w\rrbracket_I(\sigma)=(\tau,o,\sigma')$, where $\tau$ is the concrete suffix trace and $\mathsf{Obs}$ projects its declared visible observation.

**Definition 1 (causal state-mediated word family).** A word $w$ is causal at $(P,\ell)$ when $P\in L_V$, $w\in W^\ell_{\mathrm{run}}(P)$, and there are $\sigma_0,\sigma_1\in\mathsf{Reach}_I(P,\ell)$ for which
$\llbracket w\rrbracket_I(\sigma_i)=(\tau_i,o_i,\sigma_i')$ are both defined and terminating in the same $\mathsf{Env}$ instance, and
...
Fix a deterministic discipline $D$ with a residual-state carrier $S_D$, an adequate projection $s_D:\Sigma\to S_D$, and one common admitted word universe $\mathcal W_D\subseteq\Sigma_D^*$. Every environment choice that affects a transition is fixed by $D$ or included in the state. The partial word semantics is continuation-closed: if an admitted prefix reaches a state and an admitted suffix is enabled there, their concatenation is in $\mathcal W_D$. For $r,r'\in S_D$, define

$$
r\sim_D r'
\quad\Longleftrightarrow\quad
\text{every }w\in\mathcal W_D\text{ has the same definedness and complete output trace from }r\text{ and }r'.
$$

Continuation closure makes $\sim_D$ a right congruence. If $Q_D=S_D/{\sim_D}$ is finite, concrete execution induces a partial Mealy transducer over quotient states. This is a semantic quotient, not a verifier abstraction.

For a concrete set $F$, let $\mathcal W_D(F)\subseteq\mathcal W_D$ be the common continuations admitted from every state in $F$.
...
and that each word in $\mathcal W_D(F)$ is total on $F$. Define
...
If the cells form a partition with unique report map $\pi_R$, this is equivalent to $\beta_D=h\circ\pi_R$ on $F$ for some $h$.

*Proof.* A cell is collision-free exactly when all representatives it covers lie in one future-observation class. This is the cardinality condition and defines $h$ on each nonempty cell. Under a partition, equal report labels imply equal quotient classes exactly when $\beta_D$ factors through $\pi_R$. ∎

**Definition 2 (report-relative residual language).** Fix $D$ and a fiber $F$ satisfying Proposition 1. A tagged word $(P,\ell,a^\#,w)$ is report-relative residual on $(D,F)$ when:
...
$L_{\mathrm{res}}^R(V,I,\mathsf{Report};K_{\mathrm{obs}})$ is the union of these tagged words over admissible $(D,F)$. Different observations under admitted $w$ imply the displayed quotient-class inequality; writing it explicitly binds the definition to Proposition 1. This is a frontier collision, not a claim that a complete whole-program report can never recover the final function.
...
A causal word can expose one distinction without supporting repeated computation. Fix a deterministic gate discipline $D$, an adequate projection $s_D$, and a canonical reset class $q_0\subseteq S_D$. A gate basis $(P_G,\mathit{reset},G,\mathit{observe},D)$ satisfies:
...
**E3 — reset.** From every admissible concrete state, $\mathit{reset}$ is defined, terminates in a state whose projection lies in $q_0$, and removes prior-gate dependence except for declared wire cells.
...
Let $\mathsf{PhysRun}_U(c)=(s,k,\mu)$ contain final status, completed-iteration count, and physical map state. Define the status-masked interface

$$
\mathsf{Run}_U(c)=
\begin{cases}
(s,\mathsf{WireObs}(\mu)),&s=\mathsf{OK},\\
(s,\bot),&s\ne\mathsf{OK}.
\end{cases}
$$
...
1. $P_U\in L_V$ independently of host configuration;
2. every valid $\mathsf{Enc}_U(d,x)$ establishes the constant/input base frame, executes exactly callback positions $0,\ldots,n(d)-1$ in order, and terminates with $\mathsf{PhysRun}_U=(\mathsf{OK},n(d),\mu)$;
...
$$
\mathsf{Run}_U(\mathsf{Enc}_U(d,x))
...
*Proof sketch.* E4-D initializes the constant/input prefix. At iteration $i$, the descriptor bound makes both sources earlier cells. E3 supplies $q_0$, E2 supplies $g$ from that class, and E4-D writes it only to the canonical destination while preserving the established prefix. Prefix induction yields $\mathsf{Eval}_g$ after $n(d)$ iterations; the exact-schedule clause supplies terminal $\mathsf{OK}$. E1 establishes that the gate uses a causal state distinction but is not needed for the Boolean induction. Safety follows only from the separate soundness premise. ∎
...
The gate uses pairwise-distinct keys: sentinel $S$ and input keys $A$ and $B$. Reset deletes all three and inserts $S$, establishing occupancy one. For the first input, zero updates $S$ and one inserts fresh $A$; for the second, zero updates $S$ and one inserts fresh $B$. Existing-key update succeeds without changing occupancy. Fresh-key update succeeds below capacity and fails when the two-entry map is full [8]. The gate observes only the second update’s success predicate, not a portable error number.
...
|---|---|---|
| LangSec [1]–[4] | recognized input language and downstream interpreter | accepted artifact versus its post-acceptance operation words |
...
*Proof by countermodels.* For $A\not\Rightarrow C$, accept only $\mathit{skip}$ and reject another safe constant program: the recognizer can be sound and acceptance-incomplete with no causal operation. For $C\not\Rightarrow P$ and $R\not\Rightarrow P$, let a jointly covered state bit have one destructive distinguishing read and no reset. For $C\not\Rightarrow R$ and $P\not\Rightarrow R$, use a report whose cells exactly separate the behavioral quotient. For $P\not\Rightarrow W$, let policy explicitly permit a NAND accelerator; the same example gives $R\not\Rightarrow W$ when a report collision has no security-relevant effect. Finally, a defect-induced, policy-excluded machine can be fully described by a diagnostic report, so policy-level weird-machine status does not generally require R. ∎
...
[6] Linux Kernel Documentation, “Running BPF Programs from Userspace,” [Online]. Available: https://docs.kernel.org/bpf/bpf_prog_run.html (accessed Jul. 11, 2026).

[7] Linux Kernel Documentation, “Program Types and ELF Sections,” [Online]. Available: https://docs.kernel.org/bpf/libbpf/program_types.html (accessed Jul. 11, 2026).

[8] Linux Kernel Documentation, “BPF_MAP_TYPE_HASH, with PERCPU and LRU Variants,” [Online]. Available: https://docs.kernel.org/bpf/map_hash.html (accessed Jul. 11, 2026).

[9] Linux Kernel Documentation, “eBPF Verifier,” [Online]. Available: https://docs.kernel.org/bpf/verifier.html (accessed Jul. 11, 2026).

[10] I. Palmer, E. Rogers, and R. Adams, “Object-centric Tracing for Language-Theoretic Security in Low-Level Interfaces,” in *Twelfth Workshop on Language-Theoretic Security (LangSec), IEEE Security and Privacy Workshops*, 2026, https://langsec.org/spw26/papers/palmer-object-tracing.pdf.
...
[20] H. Lu, S. Wang, Y. Wu, W. He, and F. Zhang, “MOAT: Towards Safe BPF Kernel Extension,” in *33rd USENIX Security Symposium (USENIX Security 24)*, pp. 1153–1170, 2024, https://www.usenix.org/conference/usenixsecurity24/presentation/lu-hongyi.
