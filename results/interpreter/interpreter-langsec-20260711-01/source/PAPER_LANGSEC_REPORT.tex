...
\title{Research Report: After Acceptance: A Claim Graph for Residual Machines, Calibrated on eBPF}
...
&\text{every }w\in\mathcal W_D\text{ has the same definedness}\\
&\text{and complete output trace from }r\text{ and }r'.
\end{split}
\end{equation}
...
Fix $D$ and a fiber $F$ satisfying Proposition 1. A tagged word $(P,\ell,a^\#,w)$ is report-relative residual on $(D,F)$ when:
\begin{equation}
\begin{aligned}
...
From every admissible concrete state, $\mathit{reset}$ is defined, terminates in a state whose projection lies in $q_0$, and removes prior-gate dependence except for declared wire cells.
...
\begin{enumerate}
\item $P_U\in L_V$ independently of host configuration;
\item every valid $\mathsf{Enc}_U(d,x)$ establishes the constant/input base frame, executes exactly callback positions $0,\ldots,n(d)-1$ in order, and terminates with $\mathsf{PhysRun}_U=(\mathsf{OK},n(d),\mu)$;
...
\bibitem{ref6}
Linux Kernel Documentation, ``Running BPF Programs from Userspace,'' [Online]. Available: \url{https://docs.kernel.org/bpf/bpf_prog_run.html} (accessed Jul. 11, 2026).

\bibitem{ref7}
Linux Kernel Documentation, ``Program Types and ELF Sections,'' [Online]. Available: \url{https://docs.kernel.org/bpf/libbpf/program_types.html} (accessed Jul. 11, 2026).

\bibitem{ref8}
Linux Kernel Documentation, ``BPF\_MAP\_TYPE\_HASH, with PERCPU and LRU Variants,'' [Online]. Available: \url{https://docs.kernel.org/bpf/map_hash.html} (accessed Jul. 11, 2026).

\bibitem{ref9}
Linux Kernel Documentation, ``eBPF Verifier,'' [Online]. Available: \url{https://docs.kernel.org/bpf/verifier.html} (accessed Jul. 11, 2026).

\bibitem{ref10}
I. Palmer, E. Rogers, and R. Adams, ``Object-centric Tracing for Language-Theoretic Security in Low-Level Interfaces,'' in \emph{Twelfth Workshop on Language-Theoretic Security (LangSec), IEEE Security and Privacy Workshops}, 2026. [Online]. Available: \url{https://langsec.org/spw26/papers/palmer-object-tracing.pdf}
...
\bibitem{ref20}
H. Lu, S. Wang, Y. Wu, W. He, and F. Zhang, ``MOAT: Towards Safe BPF Kernel Extension,'' in \emph{33rd USENIX Security Symposium (USENIX Security 24)}, pp. 1153--1170, 2024. [Online]. Available: \url{https://www.usenix.org/conference/usenixsecurity24/presentation/lu-hongyi}
