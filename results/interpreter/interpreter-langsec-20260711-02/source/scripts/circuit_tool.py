#!/usr/bin/env python3
"""WMC1 encoder and separate Boolean oracle for the state-mediated circuit VM.
...
def create_full_boundary(source_path: Path, descriptor_path: Path) -> Circuit:
    """Generate the joint 64-input/512-gate/578-wire ABI boundary circuit."""
    inputs = [f"i{index}" for index in range(VM_MAX_INPUTS)]
    gates: list[dict[str, Any]] = []
    previous = "i0"
    for index in range(VM_MAX_GATES):
        gate_id = f"g{index}"
        current_input = inputs[index % VM_MAX_INPUTS]
        args = [current_input, current_input] if index == 0 else [previous, current_input]
        gates.append({"id": gate_id, "op": "nand", "args": args})
        previous = gate_id
    spec = {
        "name": "full_64_512",
        "inputs": inputs,
...
def _cmd_full_boundary(args: argparse.Namespace) -> int:
    circuit = create_full_boundary(Path(args.source), Path(args.output))
    print(json.dumps({"gate_count": circuit.gate_count,
                      "input_count": circuit.input_count,
                      "name": circuit.name,
                      "output": args.output,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    full_boundary_parser = subcommands.add_parser("full-boundary")
    full_boundary_parser.add_argument("source")
    full_boundary_parser.add_argument("output")
    full_boundary_parser.set_defaults(func=_cmd_full_boundary)
    return parser
