...
The implementation is prepared for the fresh, source-snapshotted Linux/aarch64 run
`results/interpreter/interpreter-v1-20260710-02/`; final numerical results are intentionally
withheld until that run and its independent audit complete. A fixed eBPF artifact `P_U`
...
over the complete shared map set, the interpreter serially reuses the residual gate. The pending
...
host-side and is applied only after `status=OK`. The pending fresh run checks acceptance of this
...
|---|---|---|
| recognized boundary | in-kernel verifier; concrete eBPF/JIT plus map/helper runtime; exact environment | pending `interpreter-v1-20260710-02` verifier log and environment record |
| causal residual relation | same second-insert suffix context; residual key set/occupancy differs; success bit differs | source, xlated data flow, and paired-run records after the fresh audit |
| local `Report_log` observation | the log locally presents the helper return as a scalar and explores both return-branch successors; paired runs are expected to show `0` and `<0` | verifier log plus raw-return records; **not** an extractor for `a#`, `gamma`, or `T#_op` |
...
| E4-D fixed interpreter | one fixed `wm_circuit` reads normalized core-gate, input, and control maps; it validates canonical SSA and dispatches every gate through `G0`; host code separately parses WMC1 and projects selected output wires | source snapshot and pending `interpreter-v1-20260710-02` object/audit |
| descriptor-prefix/frame invariants | canonical destinations exclude aliases; invalid ABI/count/op/destination/forward-reference controls receive an error status and a masked output | source snapshot and pending negative-suite audit |
| bounded circuit configuration domain | the host WMC1 encoder plus independent oracle drive named circuits, a fixed-seed random corpus, zero-gate, and 512-gate boundary descriptors | regenerated descriptors/corpus and pending interpreter JSONL audit |
| serial reuse regression | one loaded harness alternates descriptors serially; it is not a concurrency test and does not establish the mutual-exclusion premise | pending stress JSONL audit |
| gate `g` = NAND | successful normal per-gate traces are checked against `output=[raw_return=0]`; cap64 and sentinel controls are checked as mechanism-removing controls | pending normal/control JSONL audit |
...
by Theorem 1 is `P_U in L_V` once for the recorded object; pending the fresh run, it remains an
explicit premise rather than a final empirical result, and it is not `d in L_V` for every
...
**Bounded interpreter evaluation (pending fresh rerun).** Prior interpreter results are not used
as final evidence after the source/audit changes. The pending source-snapshotted run is
`interpreter-v1-20260710-02`. Until it completes and its independent
semantic audit and provenance checks pass, this manuscript asserts no translated-size,
instruction-count, JSONL-row-count, acceptance, or pass/fail number for that run.

The pending suite rebuilds the fixed interpreter from its source snapshot, regenerates WMC1 input
files from symbolic circuit descriptions, and records normal, capacity-control, sentinel-control,
and explicit-baseline variants. It includes named-circuit truth tables, a fixed-seed random DAG
corpus, a 512-gate chain, a zero-gate boundary, malformed core-ABI configurations, and a serial
alternating-invocation regression. Each named circuit may exhaust its own finite input set, but
the corpus and boundary tests are regression evidence only: they do not enumerate the descriptor
domain `D_{64,512}`. In particular, the alternating-invocation regression is not a concurrent
stress test and cannot establish the whole-map-set mutual-exclusion premise of E4-D.

The suite will emit per-gate raw-return traces, host-run records, an independent oracle audit, and
a self-issued SHA-256 integrity manifest over the source snapshot and generated artifacts. The
...
execution evidence is pending `interpreter-v1-20260710-02`. It does not establish artifact-
...
and experiments. The pending `interpreter-v1-20260710-02` audit has no asserted numerical result
in this draft. Named-circuit tests, bounded random corpora, and a deep boundary case are not an
