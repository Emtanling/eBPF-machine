...
- `make interpreter-data`: a fresh `results/interpreter/<run-id>/` with a
  ...
  `interpreter-v1-20260710-01` run has 37,506 JSONL rows: 25,464 per-gate
  records, 12,035 successful runs, and seven fail-closed malformed-control
  ...
  mechanism variants. The alternating sequence checks reset/no-observed-state-
  contamination under one harness; it is not a concurrent-access test.
