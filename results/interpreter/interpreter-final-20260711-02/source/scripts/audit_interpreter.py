#!/usr/bin/env python3
"""Semantic audit for the bounded state-mediated circuit interpreter evidence."""
...
def require_same_bytes(actual: Path, expected: Path, audit: Audit,
                       label: str) -> None:
    if not actual.is_file():
        audit.require(False, f"{label}: missing {actual.name}")
    elif not expected.is_file():
        audit.require(False, f"{label}: generator did not produce {expected.name}")
    else:
        audit.require(actual.read_bytes() == expected.read_bytes(),
                      f"{label}: bytes differ for {actual.name}")


def verify_named_descriptor_sources(results_dir: Path, audit: Audit) -> None:
    """Bind every named descriptor to the symbolic JSON in the source snapshot."""
    source_dir = results_dir / "source" / "circuits"
    descriptor_dir = results_dir / "descriptors"
    expected_specs = {f"{name}.json" for name in KNOWN_CIRCUITS}
    expected_descriptors = {f"{name}.wmc" for name in KNOWN_CIRCUITS}
    source_specs = {path.name for path in source_dir.glob("*.json")} \
        if source_dir.is_dir() else set()
    descriptors = {path.name for path in descriptor_dir.glob("*.wmc")} \
        if descriptor_dir.is_dir() else set()
    audit.require(source_specs == expected_specs,
                  "named descriptors: source JSON set does not match declared corpus")
    audit.require(descriptors == expected_descriptors,
                  "named descriptors: WMC set does not match declared corpus")

    with tempfile.TemporaryDirectory(prefix="wm-named-audit-") as temporary:
        generated_dir = Path(temporary)
        for name in sorted(KNOWN_CIRCUITS):
            source = source_dir / f"{name}.json"
            generated = generated_dir / f"{name}.wmc"
            try:
                circuit = circuit_tool.compile_file(source, generated)
            except (OSError, ValueError) as exc:
                audit.require(False, f"named descriptors: cannot compile {name}: {exc}")
                continue
            audit.require(circuit.name == name,
                          f"named descriptors: source name differs for {name}")
            require_same_bytes(descriptor_dir / f"{name}.wmc", generated,
                               audit, "named descriptors")


def verify_deep_boundary_source(results_dir: Path, audit: Audit) -> None:
    """Bind the declared 512-gate boundary inputs to its deterministic generator."""
    actual_json = results_dir / "boundary_deep_512.json"
    actual_wmc = results_dir / "boundary_deep_512.wmc"
    with tempfile.TemporaryDirectory(prefix="wm-boundary-audit-") as temporary:
        generated_json = Path(temporary) / "boundary_deep_512.json"
        generated_wmc = Path(temporary) / "boundary_deep_512.wmc"
        try:
            circuit = circuit_tool.create_deep_chain(
                generated_json, generated_wmc, circuit_tool.VM_MAX_GATES
            )
        except (OSError, ValueError) as exc:
            audit.require(False, f"boundary: regeneration failed: {exc}")
            return
        audit.require(circuit.name == "deep_512" and
                      circuit.gate_count == circuit_tool.VM_MAX_GATES,
                      "boundary: generator did not produce the declared deep chain")
        require_same_bytes(actual_json, generated_json, audit, "boundary")
        require_same_bytes(actual_wmc, generated_wmc, audit, "boundary")


def verify_full_boundary_source(results_dir: Path, audit: Audit) -> None:
    """Bind the joint 64-input/512-gate boundary to its deterministic generator."""
    actual_json = results_dir / "boundary_full_64_512.json"
    actual_wmc = results_dir / "boundary_full_64_512.wmc"
    with tempfile.TemporaryDirectory(prefix="wm-full-boundary-audit-") as temporary:
        generated_json = Path(temporary) / "boundary_full_64_512.json"
        generated_wmc = Path(temporary) / "boundary_full_64_512.wmc"
        try:
            circuit = circuit_tool.create_full_boundary(generated_json, generated_wmc)
        except (OSError, ValueError) as exc:
            audit.require(False, f"full boundary: regeneration failed: {exc}")
            return
        audit.require(circuit.name == "full_64_512" and
                      circuit.input_count == circuit_tool.VM_MAX_INPUTS and
                      circuit.gate_count == circuit_tool.VM_MAX_GATES and
                      circuit.wire_count == circuit_tool.VM_MAX_WIRES,
                      "full boundary: generator did not reach all declared ABI limits")
        require_same_bytes(actual_json, generated_json, audit, "full boundary")
        require_same_bytes(actual_wmc, generated_wmc, audit, "full boundary")
...
def audit_results(results_dir: Path) -> Audit:
    audit = Audit()
    verify_named_descriptor_sources(results_dir, audit)
    verify_deep_boundary_source(results_dir, audit)
    verify_full_boundary_source(results_dir, audit)
    ...
    full_boundary = load_rows(results_dir / "interpreter_full_boundary.jsonl", audit)
    verify_runs_and_gates(full_boundary, audit, "normal_full_boundary", 1, 2,
                          True, False, descriptors, expected_program_tag=normal_tag)
    full_boundary_runs = [row for row in full_boundary if row.get("record") == "run"]
    audit.require(
        len(full_boundary_runs) == 2 and
        all(row.get("circuit") == "full_64_512" and
            row.get("input_count") == circuit_tool.VM_MAX_INPUTS and
            row.get("gate_count") == circuit_tool.VM_MAX_GATES and
            row.get("kind") == "fixed_boundary" for row in full_boundary_runs) and
        sorted(row.get("assignment") for row in full_boundary_runs) ==
        [0, (1 << circuit_tool.VM_MAX_INPUTS) - 1] and
        sorted(row.get("ordinal") for row in full_boundary_runs) == [0, 1],
        "normal_full_boundary: joint input/gate/wire ABI boundary was not exercised",
    )
