...
the long-form `PAPER_DRAFT.md` and the research report
`PAPER_REPORT.md` (IEEE source: `PAPER_REPORT.tex`), plus a
numeric precision-control experiment. It is
intended for an isolated Linux VM with root access and BTF enabled.

## What this establishes

Full scope is in `ARTIFACT.md`. In the report's claim graph, the direct
result records acceptance (A) and a causal state distinction (C), and supports
bounded programmability (P) under the stated source-to-object, reset, frame,
and serialization premises. It does not establish report-relative residual
node R or policy/threat node W:
...
- A fixed accepted program `wm_circuit` implements a bounded data-parametric
  state-mediated circuit interpreter: host-parsed WMC1 text is normalized into a
...
- The interpreter revalidates ABI/count/opcode/canonical-destination/forward-
  ...
  residual gate map. Before its loop it seeds wire 0 with 0 and wire 1 with 1
  while preserving host-installed primary inputs. A nonzero status masks the
  observable wire result; stale physical map cells are not treated as an
  output.
...
stress JSONL, the separate author-run semantic audit, and a self-issued SHA-256 integrity
...
The report-bound run `interpreter-final-20260711-02` passed its separate
author-run semantic audit and integrity check: 38,533 rows comprising 26,488
per-gate records, 12,037
...
## Claim and Research Boundaries

The report distinguishes the accepted artifact language `L_V`,
ordinary runtime words `W_run(P)`, the causal tagged family `L_causal`,
the report-relative residual language `L_res^R`, and policy/threat claims.
A gap is called contract-shape-induced only after an actual computed-report
non-factorization is established relative to a declared contract. This
artifact establishes `L_causal` but has no Linux report-cell extractor and
therefore does not establish `L_res^R`. Soundness or incompleteness alone
does not imply programmability.
...
The subsequent thesis correction, countermodels, and evidence-boundary changes
are recorded in the revision notes; the independent post-revision
formal/evidence decision is retained in the review records.
