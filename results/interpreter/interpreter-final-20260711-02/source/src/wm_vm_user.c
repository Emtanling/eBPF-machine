...
static char program_tag[sizeof(((struct bpf_prog_info *)0)->tag) * 2 + 1];
...
static void format_program_tag(const struct bpf_prog_info *info)
{
    for (size_t i = 0; i < sizeof(info->tag); i++)
        snprintf(program_tag + (i * 2), 3, "%02x", info->tag[i]);
    program_tag[sizeof(info->tag) * 2] = '\0';
}
                   ...
                   const char *kind, uint32_t ordinal)
{
    ...
    /* A non-OK control status masks map contents by protocol.  In particular,
     * do not read or project possibly stale WIRES/VM_TRACE cells as a circuit
     * result after a malformed or otherwise failed interpreter invocation. */
    if (control.status != VM_STATUS_OK) {
        printf("{\"record\":\"run\","
               "\"circuit\":\"%s\",\"kind\":\"%s\",\"ordinal\":%u,\"run_seq\":%u,"
               "\"program_id\":%u,\"program_tag\":\"%s\",\"input_count\":%u,\"gate_count\":%u,"
               "\"assignment\":%llu,\"variant_id\":%u,\"gate_cap\":%llu,"
               "\"logical_expected\":%llu,\"variant_expected\":%llu,"
               "\"actual\":null,\"status\":%u,\"executed\":%u,"
               "\"failing_gate\":%u,\"gate_error_count\":%llu,"
               "\"trace_passed\":false,\"passed\":false}\n",
               image->name, kind, ordinal, control.run_seq, program_id, program_tag,
               image->input_count, image->gate_count,
               (unsigned long long)assignment, variant_id,
               (unsigned long long)gate_cap,
               (unsigned long long)logical_expected,
               (unsigned long long)expected, control.status, control.executed,
    ...
    for (uint32_t i = 0; i < image->gate_count; i++) {
        ...
        if (emit_gate_rows) {
            printf("{\"record\":\"gate\",\"circuit\":\"%s\","
                   ...
                   "\"program_id\":%u,\"program_tag\":\"%s\",\"gate\":%u,\"src0\":%u,"
                         ...
                         uint32_t expected_executed)
{
    ...
    failed = control.status != expected_status ||
             ...
             control.executed != expected_executed;
    printf("{\"record\":\"negative\",\"case\":\"%s\","
           "\"run_seq\":%u,\"program_id\":%u,\"program_tag\":\"%s\",\"expected_status\":%u,"
           "\"actual_status\":%u,\"expected_failing_gate\":%u,"
           "\"actual_failing_gate\":%u,\"expected_executed\":%u,"
           "\"actual_executed\":%u,\"passed\":%s}\n",
           name, control.run_seq, program_id, program_tag, expected_status, control.status,
           expected_failing_gate, control.failing_gate,
           expected_executed, control.executed,
           failed ? "false" : "true");
    return failed;
}

static int negative_late_descriptor_case(void)
{
    struct wm_vm_control control = {
        .abi_version = VM_ABI_VERSION,
        .input_count = 2,
    ...
    struct wm_gate_desc first = {
        .op = VM_OP_NAND,
        .src0 = 2,
        .src1 = 3,
        .dst = 4,
    };
    struct wm_gate_desc second = {
        .op = VM_OP_NAND,
        .src0 = 2,
        .src1 = 3,
        .dst = 6, /* Canonical gate-1 destination is 5. */
    };
    int failed;

    if (map_set(circuit_fd, 0, &first, "CIRCUIT") ||
        map_set(circuit_fd, 1, &second, "CIRCUIT") ||
        map_set(control_fd, 0, &control, "VM_CONTROL") || run_program() ||
        map_get(control_fd, 0, &control, "VM_CONTROL"))
        return 1;
    failed = control.status != VM_STATUS_BAD_DESCRIPTOR ||
             control.failing_gate != 1 || control.executed != 1;
    printf("{\"record\":\"negative\",\"case\":\"bad_late_dst\","
           "\"run_seq\":%u,\"program_id\":%u,\"program_tag\":\"%s\",\"expected_status\":%u,"
           "\"actual_status\":%u,\"expected_failing_gate\":1,"
           "\"actual_failing_gate\":%u,\"expected_executed\":1,"
           "\"actual_executed\":%u,\"passed\":%s}\n",
           control.run_seq, program_id, program_tag, VM_STATUS_BAD_DESCRIPTOR, control.status,
           control.failing_gate, control.executed, failed ? "false" : "true");
    return failed;
}
...
static int run_negative_suite(void)
{
    ...
    failures += negative_case("forward_reference", base, &g,
                              VM_STATUS_BAD_DESCRIPTOR, 0, 0);
    failures += negative_late_descriptor_case();
    return failures;
}
...
static int parse_assignment(const char *text, uint64_t *assignment)
{
    char *end = NULL;
    ...
    if (errno == ERANGE || !end || *end ||
        (unsigned long long)(uint64_t)value != value)
        return 1;
    *assignment = (uint64_t)value;
    return 0;
}

static int run_fixed_assignments(const char *path, int assignment_count,
                                 char **assignment_texts)
{
    struct circuit_image *image = calloc(1, sizeof(*image));
    int failures = 0;
    ...
    for (int index = 0; index < assignment_count; index++) {
        ...
        if (parse_assignment(assignment_texts[index], &assignment)) {
            fprintf(stderr, "%s: invalid unsigned 64-bit assignment: %s\n",
                    path, assignment_texts[index]);
            free(image);
            return 1;
        }
        failures += run_one(image, assignment, "fixed_boundary",
                            (uint32_t)index);
    }
    free(image);
    return failures;
...
static void usage(const char *argv0)
{
    fprintf(stderr,
            ...
            "       %s fixed FILE.wmc ASSIGNMENT [ASSIGNMENT ...]\n"
            "       %s negative\n",
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "run")) {
        ...
        if (argc < 4) {
            ...
            failures = run_fixed_assignments(argv[2], argc - 3, &argv[3]);
        }
    ...
    fprintf(stderr, "wm_circuit: program_id=%u program_tag=%s failures=%d\n",
            program_id, program_tag, failures);
    bpf_object__close(obj);
    return failures ? 1 : 0;
}
