...
**Terminology.** `L_V` contains accepted program artifacts. `W_run(P)`
...
- `make interpreter-data`: a fresh `results/interpreter/<run-id>/` with a
  ...
  `interpreter-final-20260711-02` run has 38,533 JSONL rows: 26,488 per-gate
...
### A.5 Coverage and separate author-run oracle audit
...
The audit is a separately implemented author-run oracle: `audit_results.py` recomputes the expected
...
The long formal development is in `PAPER_DRAFT.md`; the claim graph and
calibration report are in `PAPER_REPORT.md`; the legacy artifact
mapping is in `results/exploitable_gap.md`.
...
the `L_causal` witness is the common second-update suffix at the internal
frontier after the first input operation. No Linux report-cell extractor is
provided, so this does not establish report-relative `L_res^R`.
...
discipline. The verifier's acceptance unit is the fixed `P_U` artifact rather
than each descriptor; the harness may reload that artifact across datasets.
No `d` is thereby in `L_V`. The run validates named circuits, random DAGs, a
