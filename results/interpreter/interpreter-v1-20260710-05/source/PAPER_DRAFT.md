...
> core descriptor `d` and its mandatory, range-checked but host-only output list `pi`. The output
...
>    Definition 7, invocation from `Enc_U(d,x)` schedules and successfully completes exactly the
>    callback positions `i=0,...,n(d)-1`, in order and without per-gate intervention by an
>    external oracle, then reports `PhysRun_U(Enc_U(d,x))=(OK,n(d),mu)` for some physical state
>    `mu`; this loop-control clause does not itself assert `WireObs(mu)=Eval(d,x)`;
