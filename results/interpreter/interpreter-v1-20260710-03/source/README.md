...
**residual-transducer artifact with a locally verifier-unresolved readout**, not a demonstrated weird
machine:
...
The current recorded run `interpreter-v1-20260710-02` passed its independent
audit and integrity check: 37,507 rows comprising 25,464 per-gate records,
12,035 successful run records, and eight fail-closed negative controls. It
covers 100 fixed-seed random DAGs, a 512-gate boundary chain, a zero-gate
descriptor, 10,000 alternating invocations, and `GATE_CAP=64`, sentinel, and
explicit-logic controls. The audit regenerates the corpus byte-for-byte and
requires each runtime BPF program tag to match the captured preserved variant.
