...
- One fixed accepted interpreter, reset discipline, and explicit canonical
  ...
  and a 512-gate chain are runtime descriptions executed by the same object.
...
- A recognizer-relative weird machine additionally needs an intended policy,
  ...
  **residual transducer with a locally verifier-unresolved readout**, not as a demonstrated weird machine.
...
- `make interpreter-data`: a fresh `results/interpreter/<run-id>/` with a
  ...
  `interpreter-v1-20260710-02` run has 37,507 JSONL rows: 25,464 per-gate
  records, 12,035 successful runs, and eight fail-closed malformed-control
  records. It includes 100 fixed-seed random DAGs, a 512-gate chain, a zero-
  gate descriptor, 10,000 **serial** alternating invocations, and three
  mechanism variants. Its audit regenerates the fixed-seed corpus byte-for-byte
  and cross-checks runtime BPF program tags against each captured variant. The
...
- `descriptors/`, `corpus/`, and the generated 512-gate boundary source and
  descriptor;
...
Theorem 1 therefore ranges over `d in D_{64,512}` under a frozen normalized
configuration and globally serializable/mutually exclusive shared-map
...
declared 512-gate boundary, malformed controls, and repeated **serial**
descriptor changes. It provides regression evidence for the declared bounded
...
Theorem 2 is a separate conditional Q-certificate result. It requires a fixed
sound report extractor, an expressible nonconstant graph, composed local
formulas, a persistent wrong-output model, and a checked dominance obligation
against the whole-program report. The Linux evidence has no machine-checked
extractor or persistent-alternative certificate, so no global Q-opacity claim
is made.
...
a residual transducer with a locally verifier-unresolved readout.
...
chain, a zero-gate constant descriptor, seven malformed map controls, and
10,000 serial alternating invocations. `VM_TRACE` makes every successful gate's raw
second update visible; for the residual build the auditor requires
`output == [raw_return == 0]`, while the explicit baseline requires no such
helper trace. The two mechanism-removing variants require every exercised gate
to become one and require at least one final-output divergence from the
logical-NAND oracle.

The run-level audit checks the JSONL graph, trace completeness, same-program-ID
discipline within each batch, negative statuses, canonical run sequences, and
normal/control oracle relations. The stress sequence is a serial reset and
state-contamination regression test; it is not a concurrency test. The
