...
steps. The supplied runner and suite issue these operations serially; they do
not implement locking and do not establish a concurrent safety property.
...
named circuits, a fixed-seed corpus, a 512-gate boundary chain, malformed-core
controls, and an alternating 10,000-invocation sequence. The latter is a
**serial reset/state-contamination regression**, not a race, linearizability,
or synchronization test.

`scripts/audit_interpreter.py` independently recomputes descriptor wire values
and checks run rows, gate traces, expected negative status/executed-count
behavior, descriptor/corpus coverage, and control variants. A successful audit
therefore supports the recorded bounded execution relation, subject to the
environment and serialization assumptions; it does not prove properties of
unrecorded kernels, concurrent callers, or arbitrary descriptors beyond the
mathematical premises.

The completed source-snapshotted run
`results/interpreter/interpreter-v1-20260710-02/` passed both the semantic
audit and integrity-manifest verification. Its 37,507 JSONL rows comprise
25,464 gate traces, 12,035 successful runs, and eight negative controls. The
audit also regenerates the 100-circuit fixed-seed corpus byte-for-byte and
checks each runtime JSONL program tag against the tag captured from the
preserved normal, cap64, sentinel, or baseline BPF variant. These are
...
`LANGSEC_REVISION_2026-07-10.md`, and
`LANGSEC_RE_REVIEW_2026-07-10.md` document the earlier gate-only revision
