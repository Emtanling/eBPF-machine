...
The eBPF program accepts one fixed interpreter P_U.  This tool does not emit
eBPF; it validates a named NAND DAG and serializes it into the independent,
bounded WMC1 runtime-description language consumed by P_U.
"""
...
def evaluate(circuit: Circuit, assignment: int, *, variant: str = "logical") -> list[int]:
    """Independent reference semantics; never calls the eBPF program."""
    validate_circuit(circuit)
    if assignment < 0:
