...
#include "rac_tracer_fexit.skel.h"
...
static int on_event(void *ctx, void *data, size_t size)
{
    ...
    events_seen++;
    fprintf(out,
        "{\"event\":\"prune_hit\",\"source\":\"fexit/states_equal+is_state_visited\","
...
int main(int argc, char **argv)
{
    ...
    int duration = 20;
    int opt, err = 1;
    struct rac_tracer_fexit_bpf *skel = NULL;
    ...
    skel = rac_tracer_fexit_bpf__open();
    if (!skel) {
        fprintf(stderr, "open tracer skeleton failed\n");
        goto out;
    }
    if (rac_tracer_fexit_bpf__load(skel)) {
        fprintf(stderr, "load fexit tracer failed; try the kprobe backend\n");
        goto out;
    }
    ...
    if (rac_tracer_fexit_bpf__attach(skel)) {
        fprintf(stderr, "attach fexit tracer failed; check vmlinux BTF function IDs\n");
        goto out;
    }
    ...
    fprintf(output,
            ...
            "\"backend\":\"fexit\",\"kernel_release\":\"%s\","
...
out:
    ...
    rac_tracer_fexit_bpf__destroy(skel);
    if (output)
        fclose(output);
    return err;
}
